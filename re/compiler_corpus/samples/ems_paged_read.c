...
typedef struct xms_read_request {
    u32 length;
    u16 source_handle;
    u32 source_offset;
...
extern volatile xms_read_request resource_xms_read_request;

extern void NEAR cb_ems_map_page_probe(u16 handle, u16 logical_page,
        u8 physical_page);
extern void NEAR cb_xms_move_probe(volatile xms_read_request *request);
...
int NEAR ems_paged_read_probe(u16 byte_count)
{
    ...
    if ((resource_source_is_banked & 1u) != 0
        ...
        resource_xms_read_request.source_handle = (u16)resource_xms_handle;
        ...
        resource_xms_read_request.destination =
                list_d8c_buffer + list_d8c_head_offset;
        cb_xms_move_probe(&resource_xms_read_request);
        transferred = byte_count;
    } else {
