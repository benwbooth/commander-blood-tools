/* Codegen probe for the complete alien face-bucket renderer owner. */
#include <dos.h>
...
#pragma aux xdb_port_write_u16 = \
        ...
        parm [dx] [ax] \
        modify exact []
#pragma aux xdb_alien_render_face_buckets_probe \
        modify exact [ax bx cx dx si di bp es]
#pragma aux xdb_alien_face_activate_probe \
...
void XDB_NEAR xdb_alien_render_face_buckets_probe(void)
{
    ...
    column_cell = XDB_FAR_AT(
            volatile xdb_u16,
            raster_segment,
            XDB_CROOLIS_COLUMN_OFFSET);
    framebuffer_column_cell = XDB_FAR_AT(
            volatile xdb_u16,
            raster_segment,
            XDB_CROOLIS_FRAMEBUFFER_COLUMN_OFFSET);
    bucket_cursor_cell = XDB_FAR_AT(
            volatile xdb_u16,
            raster_segment,
            XDB_CROOLIS_BUCKET_CURSOR_OFFSET);
    ...
    clipped_sort_head = XDB_FAR_AT(
            volatile xdb_u16,
            raster_segment,
            XDB_CROOLIS_CLIPPED_SORT_HEAD_OFFSET);
    active_list_root = XDB_FAR_AT(
            volatile xdb_u16,
            raster_segment,
            XDB_CROOLIS_ACTIVE_LIST_ROOT_OFFSET);
    head = XDB_FAR_AT(
    ...
    middle = XDB_FAR_AT(
            volatile xdb_alien_raster_record,
            raster_segment,
            XDB_CROOLIS_ACTIVE_LIST_MIDDLE_OFFSET);
    tail = XDB_FAR_AT(
            volatile xdb_alien_raster_record,
            raster_segment,
            XDB_CROOLIS_ACTIVE_LIST_TAIL_OFFSET);

    record_offset = XDB_CROOLIS_RASTER_POOL_OFFSET;
    ...
    *bucket_cursor_cell = XDB_CROOLIS_FACE_BUCKETS_OFFSET;
    *column_cell = 0u;
    bucket_offset = XDB_CROOLIS_FACE_BUCKETS_OFFSET;
    column = 0u;
    ...
    head->next = XDB_CROOLIS_ACTIVE_LIST_MIDDLE_OFFSET;
    head->remaining = 0x014a;
    head->edge_0_position = (xdb_i32)0x80000000ul;
    head->edge_1_position = 0;
    head->edge_0_step = 0;
    head->edge_1_step = 0;
    middle->previous = XDB_CROOLIS_ACTIVE_LIST_HEAD_OFFSET;
    middle->next = XDB_CROOLIS_ACTIVE_LIST_TAIL_OFFSET;
    ...
    tail->advance_offset = XDB_CROOLIS_ADVANCE_COLUMN_OFFSET;
    tail->remaining = -1;
    *active_list_root = XDB_CROOLIS_ACTIVE_LIST_MIDDLE_OFFSET;
    head->flags = 1u;
    head->output_end = XDB_CROOLIS_ACTIVE_LIST_HEAD_OFFSET + 0x10u;
...
next_column:
    ...
    *bucket = 0u;
    while (face_offset != 0u && *free_head != 0u) {
        ...
        xdb_alien_face_activate_probe(face, raster_segment);
        face_offset = next_offset;
    }

    if (head->next != XDB_CROOLIS_ACTIVE_LIST_MIDDLE_OFFSET) {
        edge = XDB_FAR_AT(
                volatile xdb_alien_raster_record,
                raster_segment,
                head->next);
        ...
        if ((xdb_i16)((xdb_u32)edge->edge_0_position >> 16) < 0) {
            ...
            boundary = XDB_FAR_AT(
                    volatile xdb_alien_span_boundary,
                    raster_segment,
                    XDB_CROOLIS_ACTIVE_LIST_HEAD_OFFSET + 0x10u);
...
finish_span_boundaries:
        ...
        if (continuation != XDB_CROOLIS_RENDER_FOUR_PLANES_OFFSET
                || (column & 3u) == 0u) {
            xdb_u16 framebuffer_segment;
            xdb_u16 framebuffer_offset;

            if (continuation == XDB_CROOLIS_RENDER_LINEAR_OFFSET) {
            ...
            *framebuffer_column_cell = framebuffer_offset;
            ...
            while ((next_boundary->flags & 0x8000u) == 0u) {
                ...
                row_stride = continuation == XDB_CROOLIS_RENDER_LINEAR_OFFSET
...
advance_one_record:
    ...
    if (record->advance_offset == XDB_CROOLIS_ADVANCE_SECONDARY_OFFSET) {
        ...
        record->advance_offset = XDB_CROOLIS_ADVANCE_REMOVE_OFFSET;
        record->edge_1_position = (xdb_i32)(
                (xdb_u32)record->edge_1_position
                + (xdb_u32)record->edge_1_step);
        record = XDB_FAR_AT(
    ...
    if (record->advance_offset == XDB_CROOLIS_ADVANCE_SWITCH_OFFSET) {
        record->edge_0_position = (xdb_i32)(
                (xdb_u32)record->edge_0_position
                + (xdb_u32)record->edge_0_step);
        record->depth_position = (xdb_i32)(
    ...
    if (record->advance_offset == XDB_CROOLIS_ADVANCE_REMOVE_OFFSET) {
    ...
    if (record->advance_offset != XDB_CROOLIS_ADVANCE_COLUMN_OFFSET) {
        return;
    }
