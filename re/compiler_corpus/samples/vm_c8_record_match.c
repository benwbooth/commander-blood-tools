/* Codegen probe for BLOODPRG 0x006F62; not recovered game source. */
...
#pragma aux vm_c8_record_match_probe parm [si] value [si] modify exact [ax bx dx si bp es]
#endif
extern u16 NEAR branch_fail_probe(void);

const u8 NEAR *NEAR vm_c8_record_match_probe(const u8 NEAR *script_bytes)
{
    int inverted;
    u16 record_offset;
    u16 operand;
    ...
    if ((query_mode & 1u) != 0) {
        matches = record[1] == operand && record[0] == 0x00c8u;
        if (matches == inverted) {
