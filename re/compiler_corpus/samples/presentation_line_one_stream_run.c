/* Codegen probe for BLOODPRG 0x001F10. */
...
extern volatile u8 GAME_DATA crop_enabled_probe;
extern volatile u16 vertical_offset_probe;
extern volatile u16 active_line_probe;
extern volatile char credits_voc_path_probe[];
extern volatile u8 presentation_gate_probe;
extern graphics_buffer_ptr GAME_DATA display_buffer_probe;

void FAR snd_stream_source_load_probe(volatile char NEAR *path);
...
void NEAR presentation_line_one_stream_run_probe(u16 link_target_offset)
{
    nav_choice_sound_gate_probe = 0u;
    crop_enabled_probe = 0u;
    vertical_offset_probe = 0u;
    active_line_probe = 1u;

    snd_stream_source_load_probe(credits_voc_path_probe);
    snd_stream_start_probe();
    vga_dac_clear_probe();
