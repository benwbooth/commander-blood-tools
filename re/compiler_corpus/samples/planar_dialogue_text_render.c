...
#define CLIP_HEIGHT 10u
#define FONT_HEIGHT 8u

void far planar_dialogue_text_render(
        const u8 far *text,
        u16 x,
        u16 y,
        u8 color)
{
    ...
    if (y > graphics_band_bottom_row
            || (i16)y <= (i16)(graphics_band_top_row - CLIP_HEIGHT)) {
    ...
    while ((character = *text++) != 0) {
        ...
        main_font_draw_width = (u16)(main_font_draw_width + (i16)(i8)advance);
