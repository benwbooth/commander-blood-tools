...
`cd/BPRG_C.EXE` only after each selected C routine's generated bytes match
the original bytes exactly at its fixed offset. The current proven set is the
three sprite no-ops and four GS byte-parser handlers. The resulting executable
is game-loadable, but it is intentionally conservative and is not the full C
replacement.
