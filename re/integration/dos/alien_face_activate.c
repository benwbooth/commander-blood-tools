...
volatile xdb_u16 xdb_alien_palette_segment;
...
static int write_record(xdb_u16 raster_segment)
{
    ...
    for (offset = 0u; offset < sizeof(xdb_alien_raster_record); ++offset) {
...
int main(void)
{
    ...
    *free_head = RECORD_OFFSET;
    ...
    head->next = XDB_CROOLIS_ACTIVE_LIST_TAIL_OFFSET;
    tail->edge_0_position = 0x7fffffffl;
    tail->edge_0_step = 0x7fffffffl;
    tail->previous = XDB_CROOLIS_ACTIVE_LIST_HEAD_OFFSET;
    xdb_alien_palette_segment = 0x3210u;
    ...
    return write_result("PASS alien face activate");
}
