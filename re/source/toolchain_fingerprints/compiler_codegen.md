...
| `0x00963F matrix_table_clear_2a1b` | Open Watcom emits the exact 19-byte natural-C body template | recovered `SS:0x2A1B` and end `0x2AAB` operands replace linker placeholders; the generated `RETF` leaves the original four-byte epilogue unreachable, and the fixed-layout DOSBox capture matches the known-good attract sequence |
| `0x00A2DD presentation_queue_finish` | Open Watcom emits the exact 21-byte natural-C body template | the compiler reverses the zero-count branch and tail-jumps to `close_file_d5b`; that is stack- and result-equivalent to the original `CALL` / `RET`, and paired patched/control DOSBox captures show only ordinary animation timing drift |
| `0x0078D0 presentation_mode_dispatch` | Open Watcom emits a fixed 93-byte natural-C body template | twelve recovered data operands bind the UI state, two hit rectangles, mouse coordinates, and presentation state; the sole caller immediately enters the next frame stage, and patched/control DOSBox captures are pixel-identical at all seven samples |
| `0x00721A nav_chart_list_build` | Open Watcom emits a fixed 58-byte natural-C body template | the recovered helper call and three data operands bind the active list, SS=DS output, and far record base; the generated ES result equals the caller's incoming record segment, and patched/control DOSBox captures are pixel-identical at all seven samples |
| `0x0071CF nav_kind2_target_list_build` | Open Watcom emits a fixed 35-instruction/70-byte natural-C body template | the recovered helper call and five data operands bind both lists, both excluded records, and the far record base; the helper ignores the compiler's pre-call EAX/CX difference, the sole caller ignores returned CX and upper EAX, and patched/control DOSBox captures are pixel-identical at all seven samples |
| `0x0070EE ship_3d_navigation_candidate_build` | Open Watcom emits a fixed 29-instruction/73-byte natural-C body template; the fixed-layout image binds it through a 79-byte ABI envelope | the envelope translates Watcom's BX output cursor to the original recursive helper's BP cursor, uses a same-segment far call, preserves EAX/BP/SI/DS, and restores ES:DI to the record base; seven direct shipped-domain vectors pass, while the sole caller ignores the generated BX residue; the DOSBox shadow completes with exact stable frames and ordinary moving-frame timing drift |
| `0x0061A6 ship_3d_position_field_resolve` | Open Watcom emits an exact-slot 50-instruction/106-byte natural-C body template; the fixed-layout image uses a 105-byte ABI binding | the binding removes three redundant DX kind copies for the original BX-based field helper, zeroes upper ESI and EAX, and loads the arche fallback through GS without an MZ relocation; all eight arbitrary-state vectors and all seven DOSBox samples match, and the original final RET byte remains unreachable |
| `0x0041D1 entity_flag_state_transition` | Open Watcom emits a fixed 17-instruction/27-byte natural-C body template; the fixed-layout image binds it into the original 31-byte slot | the binding restores GS:0x6212 table ownership, AX/BX preservation, and the original OR/JNS flag behavior; 3,840 exhaustive low-byte differential vectors match complete machine state, and six of seven DOSBox samples are pixel-identical with only moving-frame timing drift at 30 seconds |
...
The compiler probe now includes the authoritative far-DS:SI, named-game-data
candidate. Direct 16-bit indexing into the text destination removes both
`__PIA` calls generated by the former decayed based pointer. Open Watcom
`-3 -os -s -mh -we` compiles it warning-free to 51 instructions/123 bytes
versus 47/106 original, with 70.21 percent mnemonic-multiset overlap. Turbo C
2.01 emits 80 instructions.

The candidate is accepted for source-port integration. It preserves the signed
ID transformation, printable copy, unconsumed stop, UI gate, EMS preference,
XMS fallback, no-backend path, and helper order. Its ordinary far calls match
the separately recovered C helper definitions: the path is in DX:AX and the
XMS destination in CX:BX. Direct linkage to the original binary instead needs
narrow adapters to DS:SI and ES:DI. The original call-only `XOR EAX,EAX`, exact
register/flag residue, ambient ES destination, and out-of-contract incoming-SF
branch remain explicit machine-ABI boundaries.
