...
#include <conio.h>
#define xdb_port_write_u8(port, value) \
        ((void)outp((port), (value)))
#define xdb_port_write_u16(port, value) \
        ((void)outpw((port), (value)))
#define xdb_port_read_u8(port) ((xdb_u8)inp(port))
#define xdb_port_write_buffer_u8(port, source, count) \
    do { \
        ...
        for (xdb_port_buffer_index = 0u; \
            ...
            (void)outp((port), (source)[xdb_port_buffer_index]); \
        } \
    } while (0)
