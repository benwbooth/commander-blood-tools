        ...
        xdb_alien_method_context XDB_NEAR *context)
{
    ...
    if (context->control.state == 0) {
        ...
        ring_cursor = xdb_croolis_slot3_ring_cursor;
        xdb_croolis_slot3_timer = 7;
        ...
        xdb_croolis_slot3_ring[ring_cursor >> 3].field_004 = 0x46;
        xdb_croolis_slot3_ring[ring_cursor >> 3].field_006 = 0;

        if (--count == 0) {
            xdb_croolis_slot3_ring_cursor = (ring_cursor - 8u) & 0x03fcu;
            return;
        }

        ring_cursor -= 8u;
        if (++xdb_croolis_slot3_generation != 0) {
            context->control.state = -1;
            state->callback = xdb_croolis_slot3_update;
    ...
    if (context->control.state >= 0) {
        ...
        if (xdb_croolis_slot3_timer & 0x8000u) {
            xdb_croolis_slot3_timer = 7;
        }
    }
