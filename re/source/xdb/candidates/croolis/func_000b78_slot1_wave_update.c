        ...
        xdb_alien_method_context XDB_NEAR *context)
{
    ...
    if ((xdb_croolis_slot2_active & 1u) == 0u) {
        state->field_04e = 0;
        state->field_050 = 0x0800u;
        state->field_052 = (xdb_i16)(state->field_052 + 0x0035);
        if ((xdb_croolis_slot1_selection_state & 2u) == 0u) {
            return;
        }

        sample = (xdb_u16)(xdb_alien_method_delta + 8);
        if (sample >= 0x0080u) {
            sample = 0x007fu;
        }
        xdb_alien_method_delta = (xdb_i16)sample;
        state->owner_offset = (xdb_u16)(size_t)xdb_croolis_slot1_selected_state;
        state->callback = xdb_croolis_slot1_finish_update;
        xdb_croolis_slot1_selection_state = 0;
