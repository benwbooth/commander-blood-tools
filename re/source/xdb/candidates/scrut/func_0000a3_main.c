...
void XDB_FAR xdb_scrut_main(void)
{
    ...
    while (running != 0u) {
        ...
        xdb_scrut_mouse_camera_step();
        xdb_scrut_camera_matrix_update();
        xdb_scrut_project_primary_mesh_then_render();
        xdb_scrut_render_starfield();
        ...
        do {
            ...
            xdb_scrut_transform_and_project();
        } while (*context_offset != 0u);

        xdb_alien_control_latch = 0u;
        xdb_scrut_bucket_faces_then_render();
    ...
    xdb_scrut_vga_clear_and_sync();
    xdb_scrut_mouse_bounds_set(3000u, 200u);
