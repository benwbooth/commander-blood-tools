...
; label_comment: NAV-CHART OBJECT PICKER: walks the visible-object list at SS:0x2AD3 ([DS:0x27C1] entries; shipped SS=DS) and hit-tests records in the caller-supplied ES segment. Marker position = the object's selector-0x0B field (FIELD_OFFSETS[0x0B] = 0x18 for kinds 8 and 0x10): x at +0x18, y at +0x1A. Box size BY KIND: default (0x0C,0x0B); kind&0x100 BLACK HOLE (0x13,0x0C); kind&0x10 SHIP (0x15,0x0A). A black hole has TWO chart positions -- +0x18/+0x1A and +0x1C/+0x1E -- and uses the second when es:[obj+0x14] != es:[arche+0x22] (0x92DF..0x92F2), i.e. the two ends of the same wormhole. When both kind bits are set, the ship size overwrites the black-hole size only at the near endpoint; the far-endpoint branch skips that test. Box origin is (x-2,y-2) and both bounds are INCLUSIVE unsigned comparisons with 16-bit wrap. Returns the FIRST hit in list order, else 0. Caller 0x8FB0: with the mouse ENABLED the hit becomes the info-panel selection (0x8FF4), with it disabled the object's name is drawn as a hover label. NATURAL C: func_0092a3_nav_chart_object_pick.c
; byte_count: 151
