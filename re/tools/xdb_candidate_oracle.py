            ...
            node_states.append(
                {
                    ...
                    "radial_target_after": get_u16(data_expected, node_state + 0x58),
...
def amer_slot2_reset_vectors(entry: int) -> list[dict[str, object]]:
    module = "amer"
    image = load_image(module)
    body_size = 49
    body_hash = "d7e35b8fcb41a11a654f391fead15a119688738b4cf48b5391bfb0461604c403"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    stack_sentinel = bytes.fromhex("5aa596698778")
    cases = (
        ("random_zero", 0x4000, 0x0000),
        ("random_borrow", 0x4400, 0x0040),
        ("random_high", 0x4800, 0x8001),
        ("random_wrap", 0x4C00, 0xFFFF),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, (name, state, random_before) in enumerate(cases):
        data_before = bytearray(
            (offset * 29 + case_index * 17 + 3) & 0xFF
            for offset in range(0x10000)
        )
        put_u16(data_before, context + 0x3A, 0x1110 + case_index)
        put_u16(data_before, context + 0x40, random_before)
        put_u16(data_before, state + 0x0E, 0xBEEF)
        put_u16(data_before, state + 0x4E, 0x2220 + case_index)
        put_u16(data_before, state + 0x52, 0x3330 + case_index)
        put_u16(data_before, state + 0x54, 0x4440 + case_index)
        put_u16(data_before, state + 0x56, 0x5550 + case_index)
        put_u16(data_before, state + 0x5C, 0x6660 + case_index)
        data_expected = bytearray(data_before)

        random_after = transform_random(random_before)
        pitch_after = signed_word(random_after) >> 6
        put_u16(data_expected, context + 0x3A, 0)
        put_u16(data_expected, context + 0x40, random_after)
        put_u16(data_expected, state + 0x0E, 0x1A5C)
        put_u16(data_expected, state + 0x4E, pitch_after)
        put_u16(data_expected, state + 0x52, 0)
        put_u16(data_expected, state + 0x54, 60)
        put_u16(data_expected, state + 0x56, 32)
        put_u16(data_expected, state + 0x5C, 0)
        ...
        vectors.append(
            {
                ...
                "entry": entry,
                "random_before": random_before,
                "random_after": get_u16(data_expected, context + 0x40),
                "velocity_x_after": signed_word(
                    get_u16(data_expected, context + 0x3A)
                ),
                ...
                "radial_after": get_u16(data_expected, state + 0x54),
                "countdown_after": get_u16(data_expected, state + 0x56),
                "behavior_seed_after": get_u16(data_expected, state + 0x5C),
                "callback_after": get_u16(data_expected, state + 0x0E),
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
        )
...
def amer_slot2_setup_transition_vectors(
    ...
    for case_index, (name, state, callback_before, phase_before) in enumerate(cases):
        ...
        put_u16(data_expected, state + 0x58, radial_target)
        ...
        vectors.append(
            {
                ...
                "callback_after": get_u16(data_expected, state + 0x0E),
                "radial_target_before": phase_before,
...
def amer_slot2_common_update_vectors(entry: int) -> list[dict[str, object]]:
    module = "amer"
    image = load_image(module)
    body_size = 438
    body_hash = "e56a182c0ba482e63a1b97cf0b19ba137f36df4c06c0aeea54ac64984c2fa3f1"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    stack_sentinel = bytes.fromhex("5aa596698778")
    cases = (
        ("outside_x_low_negative_roll", -41, 0, 0, -48, 0, 100, 10),
        ("outside_x_high_middle_roll", 40, 0, 0, 0, 0, 900, 20),
        ("outside_y_low_positive_roll", 0, -41, 0, 40, 0, 1000, 30),
        ("outside_y_high", 0, 41, 0, 5, -3, 0xFFFF, 40),
        ("outside_z_high", 0, 0, 41, -5, 3, 0x03F0, 50),
        ("outside_z_low", 0, 0, -80, 31, 1, 0x0380, 60),
        ("wrapped_negative_roll", 41, 0, 0, 0x7FE0, 0, 0x0320, 65),
        ("camera_facing", 0, 0, -1, 7, 0, 0x0200, 70),
        ("handoff_clamped_depth", 0, 0, 0, 9, 0, 0x0100, 80),
        ("handoff_moving_depth", -40, 40, 40, 11, 0, 0x0001, 90),
    )
    vectors: list[dict[str, object]] = []
    ...
    def follower_snapshot(memory: bytes | bytearray) -> list[dict[str, int]]:
        return [
            {
                "pitch": get_u16(memory, state + node_index * 0x5E + 0x4E),
                "roll": get_u16(memory, state + node_index * 0x5E + 0x52),
            }
            for node_index in range(1, 5)
        ]

    for case_index, case in enumerate(cases):
        name, camera_x, camera_y, camera_z, roll, velocity_x, phase, radial = case
        target_radial = 20 + case_index * 7
        pan = (0x1200 + case_index * 0x0111) & 0xFFFF
        camera_pan = (0x0200 + case_index * 0x0130) & 0xFFFF
        camera_depth_step = (20, 60, -64, 127, 256, -1, 72, 40, 20, 200)[case_index]
        camera_matrix = [
            0x00012000 + case_index * 0x100,
            -0x0000E000 + case_index * 0x80,
            0x00018000 - case_index * 0x40,
        ]
        camera_position = [
            0x00110000 + case_index * 0x1000,
            -0x00090000 + case_index * 0x0800,
            0x00200000 - case_index * 0x0400,
        ]
        velocity_before = [velocity_x, 0x2222, 0x3333]
        position_before = [
            0x11110000 + case_index,
            0x22220000 + case_index,
            0x33330000 + case_index,
        ]
        data_before = bytearray(
            (offset * 29 + case_index * 17 + 3) & 0xFF
            for offset in range(0x10000)
        ...
        put_u16(data_before, context + 0x36, 0xA55A)
        put_u16(data_before, context + 0x38, 0x2468)
        for offset, value in zip((0x3A, 0x3C, 0x3E), velocity_before):
            put_u16(data_before, context + offset, value)
        put_u16(data_before, context + 0x42, phase)
        put_u16(data_before, state + 0x0E, 0xBEEF)
        put_u16(data_before, state + 0x38, camera_x)
        put_u16(data_before, state + 0x3C, camera_y)
        put_u16(data_before, state + 0x40, camera_z)
        for offset, value in zip((0x42, 0x46, 0x4A), position_before):
            put_u32(data_before, state + offset, value)
        put_u16(data_before, state + 0x50, pan)
        put_u16(data_before, state + 0x52, roll)
        put_u16(data_before, state + 0x54, radial)
        put_u16(data_before, state + 0x58, target_radial)
        for node_index in range(1, 5):
            node = state + node_index * 0x5E
            put_u16(data_before, node + 0x4E, 0x4000 + node_index)
            put_u16(data_before, node + 0x52, 0x5000 + node_index)
        put_u16(data_before, 0x001E, 0x7777)
        put_u16(data_before, 0x22F8, camera_pan)
        put_u16(data_before, 0x22FC, camera_depth_step)
        for offset, value in zip((0x22D2, 0x22D6, 0x22DA), camera_matrix):
            put_u32(data_before, offset, value)
        for offset, value in zip((0x22EA, 0x22EE, 0x22F2), camera_position):
            put_u32(data_before, offset, value)
        data_expected = bytearray(data_before)
        code_before = bytearray(image)
        active_before = 0x7000 + case_index
        put_u16(code_before, active_offset, active_before)
        code_expected = bytearray(code_before)

        radial_delta = (target_radial - radial) & 0xFFFF
        radial_after = (radial + (signed_word(radial_delta) >> 3)) & 0xFFFF
        put_u16(data_expected, state + 0x54, radial_after)
        in_center = (
            -40 <= camera_x < 40
            and -40 <= camera_y <= 40
            and -80 < camera_z <= 40
        )
        if in_center and camera_z < 0:
            path = "camera_facing"
            put_u16(data_expected, state + 0x50, (camera_pan + 0x0800) & 0x0FFC)
        elif in_center:
            path = "return_started"
            put_u16(data_expected, state + 0x54, 0)
            generated_position = []
            for matrix_value, position_value in zip(camera_matrix, camera_position):
                product = ((matrix_value & 0xFFFFFFFF) * (-160 & 0xFFFFFFFF))
                combined = (product + (position_value & 0xFFFFFFFF)) & 0xFFFFFFFF
                generated = -(
                    signed_dword(combined) >> 16
                )
                generated_position.append(generated & 0xFFFFFFFF)
            for offset, value in zip((0x42, 0x46, 0x4A), generated_position):
                put_u32(data_expected, state + offset, value)
            depth = signed_word(camera_depth_step) >> 1
            depth = max(depth, 40)
            scale = ((depth & 0xFFFF) >> 2) + 20
            velocity_after = [
                signed_dword((value & 0xFFFFFFFF) * depth) >> 18
                for value in camera_matrix
            ]
            put_u16(data_expected, context + 0x36, 0x8001)
            put_u16(data_expected, context + 0x38, scale)
            for offset, value in zip((0x3A, 0x3C, 0x3E), velocity_after):
                put_u16(data_expected, context + offset, value)
            put_u16(data_expected, state + 0x0E, 0x18D3)
            put_u16(data_expected, 0x001E, 1)
            put_u16(data_expected, 0x22FC, -64)
            put_u16(code_expected, active_offset, 1)
        else:
            path = "motion"
            roll_after = (roll + velocity_x) & 0xFFFF
            put_u16(data_expected, state + 0x52, roll_after)
            put_u16(
                data_expected,
                state + 0x50,
                pan + (signed_word(roll_after) >> 3),
            )
            phase_after = (phase + 132) & 0x03FF
            put_u16(data_expected, context + 0x42, phase_after)
            negative_phase = (-phase_after) & 0xFFFF
            roll_region = signed_word((roll_after + 32) & 0xFFFF)
            if roll_region < 0:
                follower_values = (
                    (-256, phase_after),
                    (256, negative_phase),
            ...
            elif signed_word((roll_region - 64) & 0xFFFF) >= 0:
                follower_values = (
                    (-512, 0),
                    (512, 0),
                    (-256, negative_phase),
            ...
            for node_index, (pitch_after, roll_value) in enumerate(
                follower_values, start=1
            ):
                node = state + node_index * 0x5E
                put_u16(data_expected, node + 0x4E, pitch_after)
                put_u16(data_expected, node + 0x52, roll_value)
        ...
        vectors.append(
            {
                ...
                "entry": entry,
                "path": path,
                "camera_translation_before": [
                    value & 0xFFFF for value in (camera_x, camera_y, camera_z)
                ],
                "camera_pan": camera_pan,
                "camera_matrix": [value & 0xFFFFFFFF for value in camera_matrix],
                "camera_position": [value & 0xFFFFFFFF for value in camera_position],
                "camera_depth_step_before": camera_depth_step & 0xFFFF,
                "camera_depth_step_after": get_u16(data_expected, 0x22FC),
                "phase_timer_before": get_u16(data_before, context + 0x38),
                "phase_timer_after": get_u16(data_expected, context + 0x38),
                "velocity_before": [
                    signed_word(get_u16(data_before, context + offset))
                    for offset in (0x3A, 0x3C, 0x3E)
                ],
                "velocity_after": [
                    signed_word(get_u16(data_expected, context + offset))
                ...
                "animation_phase_before": phase,
                "animation_phase_after": get_u16(data_expected, context + 0x42),
                ...
                "roll_after": get_u16(data_expected, state + 0x52),
                "radial_before": radial & 0xFFFF,
                "radial_target": target_radial,
                "radial_after": get_u16(data_expected, state + 0x54),
                "followers_before": follower_snapshot(data_before),
                "followers_after": follower_snapshot(data_expected),
                "callback_after": get_u16(data_expected, state + 0x0E),
                "active_before": active_before,
                "active_after": get_u16(code_expected, active_offset),
                "callback_countdown_before": get_u16(data_before, 0x001E),
                "callback_countdown_after": get_u16(data_expected, 0x001E),
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
        )
...
def amer_slot2_update_head_vectors(entry: int) -> list[dict[str, object]]:
    module = "amer"
    image = load_image(module)
    body_size = 139
    body_hash = "4c3ed275fcb6c9fc02a012635361ab7483f2b715637e5bd7ca0d4e6b1a18d280"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    state = 0x4000
    return_address = 0xF000
    stack_sentinel = bytes.fromhex("788796695aa5")
    transfer_entries = {
        "common": 0x171D,
        "selection": 0x193E,
        "reset": 0x1A2B,
    ...
    cases = (
        ("selection_requested", 0, 1, 9, 0, 0, 0, 0, 0, 0x1234),
        ("selected_does_not_request", 0, 2, 2, 0, 0, 0, 0, 0, 0x2345),
        ("negative_delta_suppresses_selection", -1, 1, 1, 0, 0, 0, 0, 0, 0x3456),
        ("countdown_sign_wrap", -1, 0, 0x8000, 0, 0, 0, 0, 0, 0x4567),
        ("camera_z_high", -1, 0, 0, 0, 0, 1501, 0, 0, 0x5678),
        ("camera_z_low", -1, 0, 0, 0, 0, -1001, 0, 0, 0x6789),
        ("camera_x_high", -1, 0, 0, 1501, 0, 0, 0, 0, 0x789A),
        ("camera_x_low", -1, 0, 0, -1501, 0, 0, 0, 0, 0x89AB),
        ("negative_random_middle_pitch", -1, 0, 0, 25, 100, -25, 20, 7, 0),
        ("zero_random_high_pitch", -1, 0, 0, 30, 1000, -30, 1000, -9, 0x1FF8),
        ("positive_random_low_pitch", -1, 0, 0, -30, -1000, 30, -1000, 11, 4),
        ("inclusive_bounds", -1, 0, 0, -1500, 16, 1500, -32, 13, 0xABCD),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        put_u16(data_before, context + 0x40, random_value)
        put_u16(data_before, state + 0x38, camera_x)
        put_u16(data_before, state + 0x3C, camera_y)
        put_u16(data_before, state + 0x40, camera_z)
        put_u16(data_before, state + 0x4E, pitch)
        ...
        data_expected = bytearray(data_before)
        code_before = bytearray(image)
        put_u16(code_before, 0x0099, method_delta)
        put_u16(code_before, 0x0B2F, selection_state)
        for target in transfer_entries.values():
            code_before[target] = 0xC3

        if method_delta >= 0 and selection_state & 1:
            transfer = "selection"
        else:
            countdown_after = (countdown - 1) & 0xFFFF
            put_u16(data_expected, context + 0x38, countdown_after)
            if signed_word(countdown_after) >= 0:
                transfer = "common"
            elif (
                camera_z > 1500
                or camera_z < -1000
                ...
                transfer = "common"
                rotated = (
                    ((random_value & 0xFFFF) >> 3)
                    | ((random_value & 0xFFFF) << 13)
                ) & 0xFFFF
                random_after = (
                    rotated - (((random_value & 0xFFFF) >> 2) & 1)
                ) & 0xFFFF
                target_roll = (random_after & 0x07FF) - 0x03FF
                denominator = (abs(target_roll) >> 2) + 16
                roll_delta = signed_word((target_roll - roll) & 0xFFFF)
                velocity = divide_toward_zero(roll_delta, denominator)
                averaged_pitch = signed_word((camera_y + pitch) & 0xFFFF) >> 1
                clamped_pitch = max(-768, min(768, averaged_pitch))
                put_u16(data_expected, context + 0x38, denominator)
                put_u16(data_expected, context + 0x3A, velocity)
                put_u16(data_expected, context + 0x40, random_after)
                put_u16(data_expected, state + 0x4E, clamped_pitch)
                put_u16(data_expected, state + 0x58, 20)
        ...
        if transfers != [transfer_entries[transfer]]:
            raise AssertionError(
                f"{module}:{entry:#x} {name}: transfers={transfers}, expected={transfer}"
            )
        actual_data = bytes(machine.mem_read(data_segment * 16, 0x10000))
        if actual_data != bytes(data_expected):
            differences = [
        ...
        vectors.append(
            {
                ...
                "entry": entry,
                "transfer": transfer,
                "method_delta": method_delta & 0xFFFF,
...
def croolis_slot2_selection_update_vectors(entry: int) -> list[dict[str, object]]:
    ...
    image = load_image(module)
    body_size = 312
    body_hash = "4aceda013ad70ae65d3afd1810d1f4cab0fddb38775f775e36f4931ac7b63f3e"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")

    data_segment = 0x5000
    extra_segment = 0x7000
    fs_segment = data_segment
    game_segment = 0xA000
    stack_segment = 0xB000
    context = 0x3000
    return_address = 0xF000
    reset_continuation = 0x1966
    selection_state_offset = 0x0B70
    active_offset = 0x16A0
    selected_offset = 0x16A2
    control_latch_offset = 0x2282
    callback_countdown_offset = 0x001E
    view_y_offset = 0x22F0
    follower_indices = (5, 6, 7)
    follower_stride = 0x005E
    follower_position_offset = 0x004A
    follower_phase_offset = 0x0056
    defaults: dict[str, object] = {
        "state": 0x4000,
        "selection_state": 1,
        "selected": "none",
        "control_latch": "inactive",
        "active": 0,
        "duration": 0x2468,
        "camera_x": 0,
        "camera_z": 500,
        ...
        "radial": 100,
        "phase": 48,
        "callback_countdown": 0x7777,
        "callback": 0x1828,
        "follower_phases": (10, 20, 30),
        "follower_positions": (0x1111A55A, 0x2222A55A, 0x3333A55A),
    }
    cases = (
        ("selection_disabled", {"selection_state": 0, "selected": "current", "active": 1}),
        ("other_model_selected", {"selected": "other"}),
        ("negative_depth_rejected", {"camera_z": -1}),
        ("depth_high_rejected", {"camera_z": 1501}),
        ("forward_limit_rejected", {"forward_z": -20479}),
        ("lateral_high_rejected", {"camera_x": 501}),
        ("lateral_low_rejected", {"camera_x": -501}),
        ("control_latch_handoff", {"control_latch": "current", "active": 0}),
        ("tracking_negative_score", {"selected": "current", "phase": 48}),
        ("tracking_positive_score", {"forward_x": -1, "phase": 32}),
        ("roll_high_clamp", {"roll": 750}),
        ("roll_low_clamp", {"forward_x": -1, "roll": -750}),
        ("pitch_high_clamp", {"position_y": 0x12341B58, "view_y": 1000}),
        ("pitch_low_clamp", {"position_y": 0x1234E4A8, "view_y": -1000}),
        ("phase_wrap_requests_callback", {"phase": 0, "callback_countdown": 9}),
        ("wrapped_state_offsets", {"state": 0xFE00, "forward_x": -1, "phase": 17}),
    )
    vectors: list[dict[str, object]] = []
    ...
    def selected_value(kind: str, state: int) -> int:
        if kind == "none":
            return 0
        if kind == "current":
            return state
        if kind == "other":
            return (state + follower_stride) & 0xFFFF
        raise AssertionError(f"unknown selected kind {kind}")

    for case_index, (name, overrides) in enumerate(cases):
        case = defaults | overrides
        state = int(case["state"])
        selection_state = int(case["selection_state"])
        selected = str(case["selected"])
        control_latch = str(case["control_latch"])
        ...
        forward_x = int(case["forward_x"])
        forward_z = int(case["forward_z"])
        position_y = int(case["position_y"])
        view_y = int(case["view_y"])
        pitch = int(case["pitch"])
        pan = int(case["pan"])
        roll = int(case["roll"])
        radial = int(case["radial"])
        phase = int(case["phase"])
        callback_countdown = int(case["callback_countdown"])
        callback = int(case["callback"])
        follower_phases = tuple(int(value) for value in case["follower_phases"])
        follower_positions = tuple(int(value) for value in case["follower_positions"])
        ...
        put_u16(data_before, state + 0x0E, callback)
        put_u16(data_before, state + 0x4E, pitch)
        put_u16(data_before, state + 0x50, pan)
        put_u16(data_before, state + 0x52, roll)
        put_u16(data_before, state + 0x54, radial)
        put_u16(data_before, state + 0x56, phase)
        for node_index, source, position in zip(
            follower_indices, follower_phases, follower_positions
        ):
            node = state + node_index * follower_stride
            put_u16(data_before, node + follower_phase_offset, source)
            put_u32(data_before, node + follower_position_offset, position)
        put_u16(data_before, control_latch_offset, context if control_latch == "current" else 0)
        put_u16(data_before, callback_countdown_offset, callback_countdown)
        put_u16(data_before, view_y_offset, view_y)
        data_expected = bytearray(data_before)
        code_before = bytearray(image)
        struct.pack_into("<H", code_before, selection_state_offset, selection_state)
        struct.pack_into("<H", code_before, active_offset, active)
        selected_before = selected_value(selected, state)
        struct.pack_into("<H", code_before, selected_offset, selected_before)
        code_expected = bytearray(code_before)
        ...
        if selection_state & 3 == 0:
            ...
            clear_selected = True
            put_u16(data_expected, state + 0x0E, 0x1727)
            put_u16(data_expected, context + 0x38, 0)
            struct.pack_into("<H", code_expected, active_offset, 0)
        elif selected == "other":
            reset_requested = True
        else:
            bounds_valid = (
                (camera_z & 0xFFFF) <= 1500
                and signed_word(forward_z) <= -20480
                and -500 <= signed_word(camera_x) <= 500
            )
            if not bounds_valid:
                reset_requested = True
                clear_selected = True
            elif control_latch == "current":
                reset_requested = True
                clear_selected = True
                struct.pack_into("<H", code_expected, active_offset, 1)

        score = None
        if reset_requested:
            if clear_selected:
                struct.pack_into("<H", code_expected, selected_offset, 0)
            for node_index, source in zip(follower_indices, follower_phases):
                node = state + node_index * follower_stride
                put_u16(data_expected, node + follower_position_offset, source)
            continuation = "reset"
            target = reset_continuation
        else:
            struct.pack_into("<H", code_expected, selected_offset, state)
            score = (
                signed_word(camera_x) * forward_z
                - (signed_word(camera_z) + 100) * forward_x
            ) & 0xFFFFFFFF
            turn = 48 if score & 0x80000000 else -48
            roll_after = max(-768, min(768, signed_word((roll + turn) & 0xFFFF)))
            put_u16(data_expected, state + 0x52, roll_after)
            put_u16(data_expected, state + 0x50, pan + (roll_after >> 4))
            half_height = signed_word((position_y + view_y) & 0xFFFF) >> 1
            pitch_delta = signed_word((half_height - pitch) & 0xFFFF)
            pitch_after = signed_word((pitch + (pitch_delta >> 2)) & 0xFFFF)
            pitch_after = max(-768, min(768, pitch_after))
            put_u16(data_expected, state + 0x4E, pitch_after)
            radial_delta = signed_word((200 - radial) & 0xFFFF)
            put_u16(data_expected, state + 0x54, radial + (radial_delta >> 2))
            phase_signed = signed_word((phase - 16) & 0xFFFF)
            if phase_signed < 0:
                put_u16(data_expected, callback_countdown_offset, 1)
            phase_after = phase_signed & 0x007F
            put_u16(data_expected, state + 0x56, phase_after)
            for node_index, source in zip(follower_indices, follower_phases):
                node = state + node_index * follower_stride
                put_u16(data_expected, node + follower_position_offset, source + phase_after)
        ...
        if bytes(machine.mem_read(stack_segment * 16 + 0xFF02, len(stack_sentinel))) != stack_sentinel:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack changed")
        expected_sp = 0xFF00 if reset_requested else 0xFF02
        if machine.reg_read(UC_X86_REG_SP) != expected_sp:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack pointer changed")

        selected_after_value = struct.unpack_from("<H", code_expected, selected_offset)[0]
        selected_after = (
            "none"
            if selected_after_value == 0
            else "current" if selected_after_value == state else "other"
        )
        vectors.append(
            {
                "name": name,
                ...
                "entry": entry,
                "continuation": continuation,
                "reset_distance": 1000 if reset_requested else None,
                "selection_state": selection_state,
                ...
                "radial_before": radial & 0xFFFF,
                "radial_after": get_u16(data_expected, state + 0x54),
                "phase_before": phase & 0xFFFF,
                "phase_after": get_u16(data_expected, state + 0x56),
                "callback_countdown_before": callback_countdown,
                "callback_countdown_after": get_u16(
                    data_expected, callback_countdown_offset
                ),
                "callback_before": callback,
                "callback_after": get_u16(data_expected, state + 0x0E),
                "follower_phases": list(follower_phases),
                "follower_positions_before": list(follower_positions),
                "follower_positions_after": [
                    get_u32(
                        data_expected,
                        state + node_index * follower_stride + follower_position_offset,
                    )
                    for node_index in follower_indices
                ],
                "score": score,
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
...
def croolis_slot2_selection_init_vectors(entry: int) -> list[dict[str, object]]:
    module = "croolis"
    image = load_image(module)
    body_size = 19
    body_hash = "6d49a291f0541c75cbf5f96ab342d65931ad93872ac6796d20f05892f7d88b24"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    game_segment = 0xA000
    stack_segment = 0xB000
    selection_entry = 0x1828
    active_offset = 0x16A0
    selected_offset = 0x16A2
    cases = (
        ("ordinary", 0x4000, 0xBEEF, 1, 0x4100),
        ("inactive_with_selection", 0x5000, 0xFFFF, 0, 0x5100),
        ("active_without_selection", 0x6000, 0x1234, 7, 0),
        ("wrapped_state_offsets", 0xFFC0, 0x5678, 0xFFFF, 0xFFF0),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        name, state, callback_before, active_before, selected_before = case
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        ...
        vectors.append(
            {
                ...
                "active_before": active_before,
                "active_after": 0,
                "selected_before": selected_before != 0,
...
def croolis_slot2_restart_vectors(entry: int) -> list[dict[str, object]]:
    module = "croolis"
    image = load_image(module)
    body_size = 10
    body_hash = "982012e95ec718643146d8a1d4d73df51b479dedf25b0d884d56bc01fdea2234"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    update_entry = 0x1727
    cases = (
        ("ordinary", 0x4000, 0xBEEF, 0x1357),
        ("high_values", 0x5000, 0xFFFF, 0x8000),
        ("wrapped_state_offsets", 0xFFC0, 0x1234, 0x5678),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, (name, state, callback_before, radial_target_before) in enumerate(cases):
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        put_u16(data_before, state + 0x0E, callback_before)
        put_u16(data_before, state + 0x58, radial_target_before)
        data_expected = bytearray(data_before)
        put_u16(data_expected, state + 0x0E, update_entry)
        put_u16(data_expected, state + 0x58, 100)
        extra_before = bytes(
            (offset * 13 + case_index + 3) & 0xFF for offset in range(0x10000)
        )
        fs_before = bytes(
        ...
        vectors.append(
            {
                ...
                "entry": entry,
                "next_stage": "update",
                "radial_target_before": radial_target_before,
                "radial_target_after": get_u16(data_expected, state + 0x58),
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
        )
...
def croolis_slot2_update_head_vectors(entry: int) -> list[dict[str, object]]:
    module = "croolis"
    image = load_image(module)
    body_size = 103
    body_hash = "170aadbb7b3f3e6c8b9b3375c3041a9ac959bf139ad461e449e75b34e41233db"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    selection_state_offset = 0x0B70
    common_entry = 0x178E
    selection_entry = 0x1815
    reset_entry = 0x1960
    cases = (
        ("selection_requested", 0x4000, 1, 50, 0, 0, 0x1234, 0),
        ("selection_selected", 0x4400, 2, 50, 0, 0, 0x5678, 0),
        ("countdown_remaining", 0x4800, 0, 2, 0, 0, 0x9ABC, 0),
        ("countdown_sign_wrap", 0x4C00, 0, 0x8000, 0, 0, 0xDEF0, 0),
        ("depth_low_outside", 0x5000, 0, 0, 0, -501, 0x1111, 0),
        ("depth_high_outside", 0x5400, 0, 0, 0, 2501, 0x2222, 0),
        ("lateral_low_outside", 0x5800, 0, 0, -1001, 0, 0x3333, 0),
        ("lateral_high_outside", 0x5C00, 0, 0, 1001, 0, 0x4444, 0),
        ("bounds_low_edges_negative_target", 0x6000, 0, 0, -1000, -500, 0, 0),
        ("bounds_high_edges_positive_target", 0x6400, 0, 0, 1000, 2500, 4, 0),
        ("random_borrow_and_division", 0x6800, 0, 0, 100, 100, 0x0040, 300),
        ("wrapped_state_offsets", 0xFFC0, 0, 0, -25, 75, 0xFFFF, -300),
    )
    vectors: list[dict[str, object]] = []
    ...
    def signed_quotient(numerator: int, denominator: int) -> int:
    ...
    for case_index, case in enumerate(cases):
        ...
        put_u16(data_before, context + 0x38, duration)
        put_u16(data_before, context + 0x3A, 0xA55A)
        put_u16(data_before, context + 0x42, random_before)
        put_u16(data_before, state + 0x38, camera_x)
        put_u16(data_before, state + 0x40, camera_z)
        put_u16(data_before, state + 0x52, roll)
        put_u16(data_before, state + 0x58, 0xBEEF)
        data_expected = bytearray(data_before)
        code_before = bytearray(image)
        struct.pack_into("<H", code_before, selection_state_offset, selection_state)
        target = selection_entry
        continuation = "selection"
        if selection_state & 3 == 0:
            next_duration = (duration - 1) & 0xFFFF
            put_u16(data_expected, context + 0x38, next_duration)
            target = common_entry
            continuation = "common"
            if signed_word(next_duration) < 0:
                depth_inside = ((camera_z + 500) & 0xFFFF) <= 3000
                lateral_inside = ((camera_x + 1000) & 0xFFFF) <= 2000
                if not depth_inside or not lateral_inside:
                    target = reset_entry
                    continuation = "reset"
                else:
                    random_after = transform_random(random_before)
                    desired_roll = (random_after & 0x03FF) - 0x01FF
                    absolute = abs(desired_roll)
                    denominator = (absolute >> 1) + 16
                    roll_delta = signed_word((desired_roll - roll) & 0xFFFF)
                    motion_accumulator = signed_quotient(roll_delta, denominator)
                    put_u16(data_expected, context + 0x38, denominator)
                    put_u16(data_expected, context + 0x3A, motion_accumulator)
                    put_u16(data_expected, context + 0x42, random_after)
                    put_u16(data_expected, state + 0x58, (768 - absolute) >> 3)
        ...
        vectors.append(
            {
                ...
                "selection_state": selection_state,
                "continuation": continuation,
                "duration_before": duration & 0xFFFF,
                "duration_after": get_u16(data_expected, context + 0x38),
                "camera_x_before": camera_x & 0xFFFF,
                "camera_z_before": camera_z & 0xFFFF,
                "random_before": random_before,
                "random_after": get_u16(data_expected, context + 0x42),
                "roll_before": roll & 0xFFFF,
                "motion_accumulator_before": get_u16(data_before, context + 0x3A),
                "motion_accumulator_after": get_u16(data_expected, context + 0x3A),
                "radial_target_before": get_u16(data_before, state + 0x58),
                "radial_target_after": get_u16(data_expected, state + 0x58),
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
...
def croolis_slot2_motion_vectors(entry: int) -> list[dict[str, object]]:
    ...
    image = load_image(module)
    body_size = 80
    body_hash = "3f9771607db8b021979ce6d5a245793abd7ae12b76304810cebf22bf86dad7bf"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    stack_sentinel = bytes.fromhex("877869965aa5")
    cases = (
        ("ordinary_no_carry", 0x4000, 100, 20, 40, 40, 80, 10, 7, 1000),
        ("positive_heading_carry", 0x4400, 100, 20, 0, 40, 80, 0, 8, 1000),
        ("negative_rounds_to_zero", 0x4800, 100, 20, 0, 40, 80, 0, -1, 1000),
        ("negative_without_carry", 0x4C00, 100, 20, 0, 40, 80, 0, -9, 1000),
        ("pitch_high_clamp", 0x5000, 7000, 1000, 0, 0, 0, 0, 0, 0),
        ("pitch_low_clamp", 0x5400, -7000, -1000, 0, 0, 0, 0, 0, 0),
        ("radial_wrapping_delta", 0x5800, 0, 0, 0, 0x7FF0, 0x800F, 0, 0, 0),
        ("roll_sign_wrap", 0x5C00, 0, 0, 0, 0, 0, 0x7FFF, 1, 0x7FFF),
        ("wrapped_state_offsets", 0xFEF0, -50, 25, -16, 500, 20, -30, 16, 0xFFF8),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        put_u16(data_before, context + 0x3A, roll_velocity)
        put_u16(data_before, context + 0x3C, seed)
        put_u16(data_before, state + 0x3C, camera_y)
        put_u16(data_before, state + 0x4E, pitch)
        put_u16(data_before, state + 0x50, pan)
        put_u16(data_before, state + 0x52, roll)
        put_u16(data_before, state + 0x54, radial)
        put_u16(data_before, state + 0x58, radial_target)
        for offset in (0x00AE, 0x010A, 0x0168):
            put_u16(data_before, state + offset, 0xA55A)
        data_expected = bytearray(data_before)

        pitch_delta = signed_word((camera_y + seed - pitch) & 0xFFFF)
        pitch_after = max(-768, min(768, pitch + (pitch_delta >> 3)))
        radial_delta = signed_word((radial_target - radial) & 0xFFFF)
        radial_after = (radial + (radial_delta >> 3)) & 0xFFFF
        roll_after = (roll + roll_velocity) & 0xFFFF
        half_roll = signed_word(roll_after) >> 1
        negative_half = (-half_roll) & 0xFFFF
        heading_delta = signed_word(roll_after) >> 4
        heading_carry = (roll_after >> 3) & 1
        pan_after = (pan + heading_delta + heading_carry) & 0xFFFF
        put_u16(data_expected, state + 0x4E, pitch_after)
        put_u16(data_expected, state + 0x50, pan_after)
        put_u16(data_expected, state + 0x52, roll_after)
        put_u16(data_expected, state + 0x54, radial_after)
        put_u16(data_expected, state + 0x00AE, negative_half)
        put_u16(data_expected, state + 0x010A, half_roll)
        put_u16(data_expected, state + 0x0168, negative_half)
        ...
        vectors.append(
            {
                ...
                "module": module,
                "entry": entry,
                "camera_y_before": camera_y & 0xFFFF,
                "seed_low_word": seed & 0xFFFF,
                "pitch_before": get_u16(data_before, state + 0x4E),
                "pitch_after": get_u16(data_expected, state + 0x4E),
                "radial_target": radial_target & 0xFFFF,
                "radial_before": radial & 0xFFFF,
                "radial_after": get_u16(data_expected, state + 0x54),
                "roll_velocity": roll_velocity & 0xFFFF,
                "roll_before": roll & 0xFFFF,
                "roll_after": get_u16(data_expected, state + 0x52),
                "pan_before": pan & 0xFFFF,
                "pan_after": get_u16(data_expected, state + 0x50),
                "heading_carry": heading_carry,
                "follower_pan_after": get_u16(data_expected, state + 0x00AE),
                "follower_pitch_after": [
                    get_u16(data_expected, state + 0x010A),
                    get_u16(data_expected, state + 0x0168),
                ],
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
        )
...
def scrut_slot2_restart_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 10
    body_hash = "98267be9dfb4b3a616cbd7b02c9d4a872ec7e998cec75d1bc1f33faab055a204"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
...
def scrut_slot2_update_head_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 102
    body_hash = "a98e94d4a10bd19f4190dc38329e16f1eed5b40ee80e45f360e9921300ff01f0"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    cases = (
        ...
        ("bounds_high_edges_positive_target", 0x6400, 0, 0, 1000, 2500, 4, 0),
        ("random_borrow_and_division", 0x6800, 0, 0, 100, 100, 0x0040, 300),
        ("target_complement_zero", 0x6C00, 0, 0, 0, 0, 0x0020, -32768),
        ("wrapped_state_offsets", 0xFFC0, 0, 0, -25, 75, 0xFFFF, -300),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        put_u16(data_before, state + 0x52, roll)
        put_u16(data_before, state + 0x58, 0xBEEF)
        put_u16(data_before, state + 0x5A, 0xA55A)
        data_expected = bytearray(data_before)
        code_before = bytearray(image)
        struct.pack_into("<H", code_before, selection_state_offset, selection_state)
        target = selection_entry
        ...
        if selection_state & 3 == 0:
            ...
            if signed_word(next_duration) < 0:
                ...
                if not depth_inside or not lateral_inside:
                    ...
                    random_after = transform_random(random_before)
                    target_roll = (random_after & 0x07FF) - 0x03FF
                    target_complement = 0x0400 - abs(target_roll)
                    denominator = (target_complement >> 3) + 0x20
                    roll_delta = signed_word((target_roll - roll) & 0xFFFF)
                    velocity = signed_quotient(roll_delta, denominator)
                    put_u16(data_expected, context + 0x38, denominator)
                    put_u16(data_expected, context + 0x42, random_after)
                    put_u16(data_expected, state + 0x58, target_complement >> 4)
                    put_u16(data_expected, state + 0x5A, velocity)
...
def scrut_slot2_common_dispatch_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 6
    body_hash = "da9d5fac31cb86554361661a2a31f22ed4fb8d69af3baa3ec24823a7e71cd3fe"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    for case_index, (name, state, context, latch) in enumerate(cases):
        ...
        stack_before = bytes.fromhex("877869965aa5")
        target = halted_entry if latch == context else motion_entry
        initial = {
            "eax": 0xA1A12345,
            "ebx": 0xB2B23456,
            "ecx": 0xC3C34567,
        ...
        vectors.append(
            {
                ...
                "continuation": "halted" if target == halted_entry else "motion",
                "data_sha256": hashlib.sha256(data_before).hexdigest(),
            }
        )
...
def scrut_slot2_motion_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 90
    body_hash = "19d6191609c7e9a140ed67c32825e248adc09169edee215942dd3286d68cce95"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    stack_sentinel = bytes.fromhex("877869965aa5")
    cases = (
        ("ordinary_no_carry", 0x4000, 100, 20, 40, 40, 80, 10, 15, 1000),
        ...
        ("counter_roll_minimum", 0x6400, 0, 0, 0, 0, 0, -32768, 0, 0),
        ("wrapped_state_offsets", 0xFEF0, -50, 25, -16, 500, 20, -30, 16, 0xFFF8),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        put_u16(data_before, state + 0x54, radial)
        put_u16(data_before, state + 0x58, radial_target)
        put_u16(data_before, state + 0x5A, roll_velocity)
        ...
        desired_pitch = signed_word(camera_y + seed) >> 1
        pitch_delta = signed_word(desired_pitch - pitch)
        pitch_after = signed_word(pitch + (pitch_delta >> 3))
        pitch_after = max(-768, min(768, pitch_after))
        radial_delta = signed_word(radial_target - radial)
        radial_after = (radial + (radial_delta >> 3)) & 0xFFFF
        roll_after = (roll + roll_velocity) & 0xFFFF
        heading_delta = signed_word(roll_after) >> 5
        heading_carry = (roll_after >> 4) & 1
        pan_after = (pan + heading_delta + heading_carry) & 0xFFFF
        counter_roll = (-signed_word(roll_after)) & 0xFFFF
        follower_pan = signed_word(counter_roll) >> 1
        follower_roll = signed_word(counter_roll) >> 2
        put_u16(data_expected, state + 0x4E, pitch_after)
        put_u16(data_expected, state + 0x50, pan_after)
        put_u16(data_expected, state + 0x52, roll_after)
        put_u16(data_expected, state + 0x54, radial_after)
        ...
        vectors.append(
            {
                ...
                "pan_after": get_u16(data_expected, state + 0x50),
                "heading_carry": heading_carry,
                "follower_pan_after": follower_pan & 0xFFFF,
                "follower_roll_after": follower_roll & 0xFFFF,
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
            }
        )
...
def scrut_slot2_begin_fade_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 5
    body_hash = "392df0d852bc458b925b4ccc3cd38c8f14650207e6cd773d0142eca6b3ee11ad"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    fade_entry = 0x17E6
    cases = (
        ("ordinary", 0x4000, 0xBEEF),
        ("high_callback", 0x5000, 0xFFFF),
        ("wrapped_state_offsets", 0xFFF8, 0x1234),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, (name, state, callback_before) in enumerate(cases):
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
...
def scrut_slot2_fade_update_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 28
    body_hash = "4af9369736fa34b6d76c8bc26dbcb0450073f82e97dcf3aca9bb431da8637507"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    motion_entry = 0x1787
    restart_entry = 0x1711
    cases = (
        ("positive_remaining", 5, 1),
        ("zero_remaining", 4, 1),
        ("negative_result", 3, 1),
        ("zero_before", 0, 0xFFFF),
        ("signed_minimum", 0x8000, 0x1234),
        ("signed_negative_near_minimum", 0x8003, 0x5678),
        ("signed_negative_boundary", 0x8004, 0x9ABC),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, (name, duration, active) in enumerate(cases):
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        ...
        struct.pack_into("<H", code_before, active_offset, active)
        code_expected = bytearray(code_before)
        next_duration = (duration - 4) & 0xFFFF
        if signed_word(duration) >= 4:
            continuation = "motion"
            target = motion_entry
            put_u16(data_expected, context + 0x38, next_duration)
        else:
            continuation = "restart"
            target = restart_entry
            put_u16(data_expected, context + 0x38, 0)
            struct.pack_into("<H", code_expected, active_offset, 0)
        extra_before = bytes(
            (offset * 13 + case_index + 3) & 0xFF for offset in range(0x10000)
...
def scrut_slot2_selection_damp_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 16
    body_hash = "572e53995576e48498edd2c441ddd5ae7e341ffd82b323df0b6f34844a4bd245"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    game_segment = 0xA000
    stack_segment = 0xB000
    steering_entry = 0x18D9
    approach_entry = 0x1868
    steering_return = 0x1862
    cases = (
        ("positive_remaining", 0x4000, 3, 0x1858),
        ("negative_remaining", 0x4400, -3, 0x1858),
        ("positive_to_zero", 0x4800, 1, 0x1858),
        ("negative_one_remains", 0x4C00, -1, 0x1858),
        ("zero_stays_zero", 0x5000, 0, 0xBEEF),
        ("signed_minimum", 0x5400, -32768, 0xCAFE),
        ("wrapped_state_offsets", 0xFFC0, 2, 0x1234),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, (name, state, radial, callback_before) in enumerate(cases):
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        ...
        put_u16(data_before, state + 0x54, radial)
        data_expected = bytearray(data_before)
        radial_after = signed_word(radial) >> 1
        put_u16(data_expected, state + 0x54, radial_after)
        if radial_after == 0:
            continuation = "approach"
            target = approach_entry
            expected_sp = 0xFF00
            put_u16(data_expected, state + 0x0E, approach_entry)
        else:
            continuation = "steering"
            target = steering_entry
            expected_sp = 0xFEFE
        ...
        if bytes(machine.mem_read(stack_segment * 16 + 0xFF00, len(stack_sentinel))) != stack_sentinel:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack sentinel changed")
        if machine.reg_read(UC_X86_REG_SP) != expected_sp:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack pointer changed")
        if continuation == "steering":
            if machine.mem_read(stack_segment * 16 + expected_sp, 2) != struct.pack(
                "<H", steering_return
            ):
                raise AssertionError(f"{module}:{entry:#x} {name}: call return changed")
            if machine.reg_read(UC_X86_REG_ECX) & 0xFF != 20:
                raise AssertionError(f"{module}:{entry:#x} {name}: steering shift changed")
        ...
        vectors.append(
            {
                ...
                "entry": entry,
                "continuation": continuation,
                "steering_precision": "damping" if continuation == "steering" else None,
                "radial_before": radial & 0xFFFF,
                "radial_after": get_u16(data_expected, state + 0x54),
                "next_callback": get_u16(data_expected, state + 0x0E),
                "data_sha256": hashlib.sha256(data_expected).hexdigest(),
...
def scrut_slot2_finish_setup_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 5
    body_hash = "de90e82d83a006fce7dde05484a2dc1e184cf3d07a0785f09b4cd03cfcfde814"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    for case_index, (name, state, callback_before) in enumerate(cases):
        ...
        put_u16(data_before, state + 0x0E, callback_before)
        data_expected = bytearray(data_before)
        put_u16(data_expected, state + 0x0E, finish_entry)
        extra_before = bytes(
            (offset * 13 + case_index + 3) & 0xFF for offset in range(0x10000)
        )
        fs_before = bytes(
...
def scrut_slot2_finish_update_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 120
    body_hash = "8be4c09bf2b8dd7ba844acf0172a131a0e932a77d01a3fe576059a6e37a8129a"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    selection_value_offset = 0x1690
    control_latch_offset = 0x2282
    selection_init_entry = 0x1802
    return_address = 0xF000
    stack_before = struct.pack("<H", return_address) + bytes.fromhex("877869965aa5")
    base = {
        "state": 0x4000,
        "depth": 500,
        "radial": 0,
        ...
        "selection_value": -1,
        "control_latch": "other",
        "visual_positions": ((0x11110010, 0x22220020), (0x33330030, 0x44440040)),
        "saved_positions": ((0x0101, 0x0202), (0x0303, 0x0404)),
    }
    cases = (
        {**base, "name": "depth_boundary_latch_other"},
        {
            **base,
            "name": "depth_boundary_latch_current",
            "state": 0x4400,
            "active": 0xA55A,
            "control_latch": "current",
        },
        {
            **base,
            "name": "high_depth_radial_easing",
            "state": 0x4800,
            "depth": 32767,
            "radial": -32768,
            "active": 1,
        ...
        {
            **base,
            "name": "radial_target_fixed_point",
            "state": 0x4C00,
            "depth": 700,
            "radial": 200,
        },
        {**base, "name": "middle_depth_upper", "state": 0x5000, "depth": 499},
        {
            **base,
            "name": "middle_depth_zero",
            "state": 0x5400,
            "depth": 0,
            "pitch": -32760,
        },
        {**base, "name": "negative_depth", "state": 0x5800, "depth": -1},
        {
            **base,
            "name": "signed_minimum_depth",
            "state": 0x5C00,
            "depth": -32768,
            "active": 0xFFFF,
            "selection_value": 0x7FFF,
        },
        {
            **base,
            "name": "visual_word_wrap",
            "state": 0x6000,
            "depth": 600,
            "visual_positions": ((0xAAAAFFFE, 0xBBBBFFF0), (0xCCCC0002, 0xDDDDFFFE)),
        },
        {
            **base,
            "name": "wrapped_state_offsets",
            "state": 0xFC80,
            "depth": -2,
            "visual_positions": ((0xEEEE7FFF, 0xFFFF8000), (0x1234FFFF, 0x56780001)),
            "saved_positions": ((0x8000, 0x7FFF), (0xFFFF, 0x0000)),
        },
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        pitch = int(case["pitch"])
        active = int(case["active"])
        selection_value = int(case["selection_value"])
        control_latch = str(case["control_latch"])
        visual_positions = tuple(
            tuple(int(component) for component in position)
            for position in case["visual_positions"]
        )
        saved_positions = tuple(
            tuple(int(component) for component in position)
            for position in case["saved_positions"]
        )
        ...
        put_u16(data_before, state + 0x40, depth)
        put_u16(data_before, state + 0x4E, pitch)
        put_u16(data_before, state + 0x54, radial)
        for node_index, node_offset in enumerate((0x332, 0x390)):
            put_u32(data_before, state + node_offset, visual_positions[node_index][0])
            put_u32(data_before, state + node_offset + 8, visual_positions[node_index][1])
            put_u16(data_before, state + node_offset + 20, saved_positions[node_index][0])
            put_u16(data_before, state + node_offset + 24, saved_positions[node_index][1])
        data_expected = bytearray(data_before)
        put_u16(
            data_expected,
            state + 0x332,
            get_u16(data_expected, state + 0x332) + 4,
        )
        put_u16(
            data_expected,
            state + 0x33A,
            get_u16(data_expected, state + 0x33A) + 64,
        )
        put_u16(
            data_expected,
            state + 0x390,
            get_u16(data_expected, state + 0x390) - 4,
        )
        put_u16(
            data_expected,
            state + 0x398,
            get_u16(data_expected, state + 0x398) + 80,
        )

        code_before = bytearray(image)
        struct.pack_into("<H", code_before, active_offset, active)
        struct.pack_into("<H", code_before, selection_value_offset, selection_value & 0xFFFF)
        code_before[return_address] = 0xCC
        code_expected = bytearray(code_before)
        struct.pack_into("<H", code_expected, selection_value_offset, 1000)
        fs_before = bytearray(
            (offset * 17 + case_index + 5) & 0xFF for offset in range(0x10000)
        )
        put_u16(
            fs_before,
            control_latch_offset,
            context if control_latch == "current" else 0x4321,
        )

        if signed_word(depth) >= 500:
            continuation = "tracking"
            target = return_address
            expected_sp = 0xFF02
            radial_delta = signed_word(200 - radial)
            put_u16(data_expected, state + 0x54, radial + (radial_delta >> 4))
            if control_latch == "current":
                struct.pack_into("<H", code_expected, active_offset, 1)
        elif signed_word(depth) >= 0:
            continuation = "descending"
            target = return_address
            expected_sp = 0xFF02
            put_u16(data_expected, state + 0x4E, pitch - 32)
        else:
            continuation = "selection_init"
            target = selection_init_entry
            expected_sp = 0xFF00
            struct.pack_into("<H", code_expected, selection_value_offset, 0)
            struct.pack_into("<H", code_expected, active_offset, 0)
            for node_index, node_offset in enumerate((0x332, 0x390)):
                put_u16(data_expected, state + node_offset, saved_positions[node_index][0])
                put_u16(data_expected, state + node_offset + 8, saved_positions[node_index][1])
        ...
        if bytes(machine.mem_read(fs_segment * 16, 0x10000)) != bytes(fs_before):
            raise AssertionError(f"{module}:{entry:#x} {name}: control segment changed")
        for segment, expected in (
            (extra_segment, extra_before),
            (game_segment, game_before),
        ):
        ...
        vectors.append(
            {
                ...
                "pitch_after": get_u16(data_expected, state + 0x4E),
                "control_latch": control_latch,
                "active_before": active & 0xFFFF,
                "active_after": struct.unpack_from("<H", code_expected, active_offset)[0],
                "selection_value_before": selection_value & 0xFFFF,
                "selection_value_after": struct.unpack_from(
                    "<H", code_expected, selection_value_offset
                )[0],
                "visual_positions_before": [
                    [component & 0xFFFFFFFF for component in position]
                    for position in visual_positions
                ],
                "visual_positions_after": [
                    [
                        get_u32(data_expected, state + node_offset),
                        get_u32(data_expected, state + node_offset + 8),
                    ]
                    for node_offset in (0x332, 0x390)
                ],
                "saved_positions": [
                    [component & 0xFFFF for component in position]
...
def scrut_slot2_selection_reset_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 52
    body_hash = "65cda118a7dc1b124b49a78b8ddb0e6f152fc75f773a1718fe8902ab432c28e7"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    cases = (
        {**base, "name": "ordinary"},
        {**base, "name": "inactive_before", "state": 0x4400, "active": 0},
        {
            **base,
            "name": "saved_signed_words",
            "state": 0x4800,
            "saved_positions": ((0x8000, 0xFFFF), (0x7FFF, 0x0000)),
        },
        {
            **base,
            "name": "preserve_position_high_words",
            "state": 0x4C00,
            "visual_positions": ((0xAAAAFFFF, 0xBBBB8000), (0xCCCC7FFF, 0xDDDD0001)),
        },
        {**base, "name": "zero_motion", "state": 0x5000, "motion_parameter": 0},
        {**base, "name": "zero_radial", "state": 0x5400, "radial_target": 0},
        {
            **base,
            "name": "high_state_words",
            "state": 0x5800,
            "active": 0xFFFF,
            "radial_target": 0xFFFF,
            "motion_parameter": -32768,
        },
        {
            **base,
            "name": "wrapped_state_offsets",
            "state": 0xFC80,
            "visual_positions": ((0xEEEE7FFF, 0xFFFF8000), (0x1234FFFF, 0x56780001)),
            "saved_positions": ((0x1357, 0x2468), (0xA55A, 0x5AA5)),
        },
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        radial_target = int(case["radial_target"])
        motion_parameter = int(case["motion_parameter"])
        ...
        for node_index, node_offset in enumerate((0x332, 0x390)):
            ...
            put_u16(data_before, state + node_offset + 24, saved_positions[node_index][1])
        put_u16(data_before, state + 0x58, radial_target)
        put_u16(data_before, state + 0x5A, motion_parameter)
        data_expected = bytearray(data_before)
        for node_index, node_offset in enumerate((0x332, 0x390)):
            put_u16(data_expected, state + node_offset, saved_positions[node_index][0])
            put_u16(data_expected, state + node_offset + 8, saved_positions[node_index][1])
        ...
        vectors.append(
            {
                ...
                "active_before": active & 0xFFFF,
                "active_after": 0,
                "radial_target_before": radial_target & 0xFFFF,
                "radial_target_after": get_u16(data_expected, state + 0x58),
                "motion_parameter_before": motion_parameter & 0xFFFF,
                "motion_parameter_after": get_u16(data_expected, state + 0x5A),
                "visual_positions_before": [
                    [component & 0xFFFFFFFF for component in position]
                    for position in visual_positions
...
def scrut_slot2_active_reset_setup_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 14
    body_hash = "2026ba4087d663f2609e9e523cc5f79238cfb095158e2b47c5f7027aff2173ea"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
...
def scrut_slot2_reset_or_camera_vectors(entry: int) -> list[dict[str, object]]:
    module = "scrut"
    image = load_image(module)
    body_size = 298
    body_hash = "dd1811c09b707e1e5c4fa0403468434b436f9045a16bbbf655cce3e785fae174"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    cases = (
        ("steering_default", {}),
        ("steering_threshold_exact", {"camera_z": -500}),
        ("steering_radial_fallback", {"forward_z": -30000}),
        ("steering_opposite_turn", {"forward_x": -2, "forward_z": -1}),
        ("steering_roll_high_clamp", {"roll": 1000}),
        ("steering_roll_low_clamp", {"roll": -1000}),
        (
            "steering_wrapped_products",
            {
                "camera_x": -32768,
                "seed": 0x7FFFFFFF,
                "forward_x": 0x7FFFFFFF,
                "forward_z": -0x80000000,
            },
        ),
        ("camera_threshold_below", {"camera_z": -501}),
        ("camera_random_zero", {"camera_z": -501, "random": 0}),
        ("camera_random_borrow", {"camera_z": -32768, "random": 64}),
        (
            "camera_negative_cosine",
            {"camera_z": -1000, "random": 0xFFFF, "cosine": -0x2345},
        ),
        ("camera_depth_wrap", {"camera_z": -501, "camera_depth_step": 0xFFFF}),
        (
            "camera_position_word_underflow",
            {
                "camera_z": -501,
                "camera_matrix": ((0, 0), (0, 0), (0, 0)),
                "camera_view": (30, 40, 50),
    ...
    for case_index, (name, overrides) in enumerate(cases):
        ...
        put_u16(data_before, context + 0x38, duration)
        put_u32(data_before, context + 0x3A, seed)
        put_u16(data_before, context + 0x42, random_before)
        put_u32(data_before, state + 0x1A, forward_x)
        put_u32(data_before, state + 0x32, forward_z)
        put_u16(data_before, state + 0x38, camera_x)
        ...
        if camera_reset:
            ...
            horizontal = signed_word(camera_z) - 2000
            vertical = signed_dword(signed_word(camera_x) - seed)
            score_radial = (
                -(
                    ((horizontal & 0xFFFFFFFF) * (forward_z & 0xFFFFFFFF))
                    + ((vertical & 0xFFFFFFFF) * (forward_x & 0xFFFFFFFF))
            ...
            shifted = signed_dword(score_radial) >> 15
            radial_after = (
                (signed_word(radial_target) >> 2) + 16 if shifted < 0 else shifted
            )
            put_u16(data_expected, state + 0x58, radial_after)
            score_turn = (
                ((vertical & 0xFFFFFFFF) * (forward_z & 0xFFFFFFFF))
            ...
            turn = 16 if score_turn & 0x80000000 else -16
            put_u16(data_expected, state + 0x5A, turn)
            put_u16(
                data_expected,
                state + 0x52,
                max(-768, min(768, signed_word(roll))),
            )
            continuation = "common"
            target = common_entry
        ...
        vectors.append(
            {
                ...
                "duration_before": duration,
                "duration_after": get_u16(data_expected, context + 0x38),
                "motion_parameter_before": motion_parameter,
                "motion_parameter_after": get_u16(data_expected, state + 0x5A),
                "pitch_before": get_u16(data_before, state + 0x4E),
                "pitch_after": get_u16(data_expected, state + 0x4E),
                "pan_before": get_u16(data_before, state + 0x50),
...
def alien_resume_texture_motion_vectors(module: str, entry: int) -> list[dict[str, object]]:
    image = load_image(module)
    configs = {
        "amer": (
            49,
            "5a55516b0dc5aa9000623a38a700941b02546abda10af5a7615608b77516d16a",
            1,
            ((0, 1), (53, -1), (35, 1), (25, -1)),
        ),
        "croolis": (
            38,
            "d0846d2653c7edb275038d45f3b0a9e3d2896a74492c0de63b33082175d4a552",
            0,
            ((0, 1), (25, -1)),
        ),
        "scrut": (
            49,
            "a4a99542e0dbb814845cf741cef70cd493f95ec4274a49401fc39ca31f048f4e",
            1,
            ((0, 1), (43, -1), (42, 1), (25, -1)),
        ),
    }
    body_size, body_hash, component, targets = configs[module]
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")

    cases = (
        ("positive_step", 0x0102, 0x2000),
        ("negative_step", 0x01FE, 0x2400),
        ("zero_step", 0x0100, 0x2800),
        ("zero_phase_repairs_step", 0x0000, 0x2C00),
        ("wrapped_high_repairs_step", 0xFF01, 0x3000),
        ("clamp_at_threshold", 0x1600, 0x3400),
        ("sum_reaches_clamp", 0x1501, 0x3800),
        ("signed_high_avoids_clamp", 0x7F01, 0x3C00),
        ("minimum_signed_step", 0x0180, 0x4000),
        ("object_offset_and_word_wrap", 0x017F, 0xFFF0),
    )
    data_segment = 0x5000
    object_segment = 0x7000
    extra_segment = 0x9000
    ...
    for case_index, (name, phase, object_offset) in enumerate(cases):
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        put_u16(data_before, 0x0002, object_segment)
        put_u16(data_before, context_offset + 0x001C, object_offset)
        put_u16(data_before, context_offset + 0x0038, phase)
        data_expected = bytearray(data_before)
        ...
        object_expected = bytearray(object_before)
        low = phase & 0xFF
        signed_delta = low if low < 0x80 else low - 0x100
        target_results = []
        for vertex, direction in targets:
            offset = (object_offset + vertex * record_size + component_offset) & 0xFFFF
        ...
        high = (((phase >> 8) & 0xFF) + low) & 0xFF
        low_after = low
        if high == 0:
            low_after = 2
        signed_high = high if high < 0x80 else high - 0x100
        if signed_high >= 22:
            low_after = 0xFE
        phase_after = (high << 8) | low_after
        put_u16(data_expected, context_offset + 0x0038, phase_after)
        ...
        actual_code = bytes(machine.mem_read(0, len(image)))
        if actual_code != image:
            differences = [
                (offset, actual_code[offset], image[offset])
                for offset in range(len(image))
                if actual_code[offset] != image[offset]
            ][:8]
            raise AssertionError(
                f"{module}:{entry:#x} {name}: code differs at {differences}"
            )
        ...
        if machine.reg_read(UC_X86_REG_SP) != 0xFF02:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack pointer changed")
        expected_registers = {
            "eax": (initial["eax"] & 0xFFFF0000) | (signed_delta & 0xFFFF),
            "edx": (initial["edx"] & 0xFFFF0000) | phase_after,
            "esi": (initial["esi"] & 0xFFFF0000) | object_offset,
            "edi": initial["edi"],
        }
        for register, expected in expected_registers.items():
            actual = machine.reg_read(REGISTERS[register])
        ...
        vectors.append(
            {
                ...
                "required_vertex_count": max(vertex for vertex, _ in targets) + 1,
                "phase_before": phase,
...
def alien_resume_timeout_vectors(module: str, entry: int) -> list[dict[str, object]]:
    image = load_image(module)
    configs = {
        "amer": (
            "cd54330606123c32becba15d2246dce8146706109af6aad33cfff812f55daa8d",
            0x0D5F,
            0x1CCF,
            1,
            ((0, 1), (53, -1), (35, 1), (25, -1)),
        ...
        "croolis": (
            "4b4f21975f3379fccef85ef31b92b2fb1dd162c59b994e107b76147fe7c13f15",
            0x0DB7,
            0x1C1B,
            0,
            ((0, 1), (25, -1)),
        ...
        "scrut": (
            "5e665638886cc5812a253a978c8d3cbc212da776776ecfd1ae52fbe9dde74ac4",
            0x0DA5,
            0x1CDB,
            1,
            ((0, 1), (43, -1), (42, 1), (25, -1)),
    ...
    body_hash, countdown_offset, final_callback, component, targets = configs[module]
    body_size = 16
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")

    cases = (
        ("ordinary_countdown", 2, 0x0102),
        ("reaches_zero", 1, 0x01FE),
        ("zero_underflows", 0, 0x0000),
        ("negative_remains_negative", 0xFFFF, 0x1600),
        ("minimum_wraps_positive", 0x8000, 0x7F01),
        ("minimum_reached", 0x8001, 0x0180),
    )
    data_segment = 0x5000
    object_segment = 0x7000
    extra_segment = 0x9000
    ...
    for case_index, (name, countdown, phase) in enumerate(cases):
        callback_before = 0x6000 + case_index * 2
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        put_u16(data_before, 0x0002, object_segment)
        put_u16(data_before, context_offset + 0x001C, object_offset)
        put_u16(data_before, context_offset + 0x0036, callback_before)
        put_u16(data_before, context_offset + 0x0038, phase)
        data_expected = bytearray(data_before)
        ...
        countdown_after = (countdown - 1) & 0xFFFF
        final_selected = bool(countdown_after & 0x8000)
        callback_after = final_callback if final_selected else callback_before
        put_u16(data_expected, context_offset + 0x0036, callback_after)
        code_before = bytearray(image)
        code_expected = bytearray(image)
        struct.pack_into("<H", code_before, countdown_offset, countdown)
        struct.pack_into("<H", code_expected, countdown_offset, countdown_after)
...
def scrut_slot2_steering_vectors(entry: int) -> list[dict[str, object]]:
    ...
    image = load_image(module)
    body_size = 121
    body_hash = "a20721a934c4580920004f000e3e848c125c790acaae44a380dcd98b9092e51b"
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered body changed")
    ...
    stack_sentinel = bytes.fromhex("877869965aa5")
    cases = (
        ("no_turn", 0x4000, 20, 0, 0, 0, 0, 100, 7),
        ("positive_score", 0x4400, 20, 1, 0, 0, 0x00100000, 100, -1),
        ("negative_score", 0x4800, 20, -1, 0, 0, 0x00100000, 100, 1),
        ("positive_half_rounds_up", 0x4C00, 20, 1, 0, 0, 0x00080000, 100, -1),
        ("negative_half_rounds_zero", 0x5000, 20, -1, 0, 0, 0x00080000, 100, 1),
        ("direction_change", 0x5400, 20, 1, 0, 0, 0x00100000, 0, 16),
        ("high_roll_clamp", 0x5800, 20, -32, 0, 760, 0x00100000, 0, 1),
        ("low_roll_clamp", 0x5C00, 20, 32, 0, -760, 0x00100000, 0, -1),
        ("steering_step_clamp", 0x6000, 20, 100, 0, 0, 0x00100000, 0, -1),
        ("approach_precision", 0x6400, 19, 1, 0, 0, 0x00080000, 0, -1),
        ("wrapped_products", 0x6800, 19, 32767, -32768, 100, 0x7FFFFFFF, -20, 1),
        ("wrapped_state_offsets", 0xFEF0, 20, -3, 2, -30, -7, 50, -16),
    )
    vectors: list[dict[str, object]] = []
    ...
    for case_index, case in enumerate(cases):
        ...
        forward_x = 0x13579BDF if camera_z == 0 else -0x2468ACE
        data_before = bytearray(
            (offset * 31 + case_index * 19 + 7) & 0xFF
            for offset in range(0x10000)
        )
        ...
        score = signed_dword(
            ((camera_x & 0xFFFFFFFF) * (forward_z & 0xFFFFFFFF))
            - ((camera_z & 0xFFFFFFFF) * (forward_x & 0xFFFFFFFF))
        )
        shifted = score >> shift
        rounding_carry = ((score & 0xFFFFFFFF) >> (shift - 1)) & 1
        rounded = ((shifted & 0xFFFF) + rounding_carry) & 0xFFFF
        turn_applied = rounded != 0
        if turn_applied:
            steering = signed_word(-rounded)
            steering = max(-32, min(32, steering))
            roll_after = signed_word(steering + roll)
            motion_after = previous_turn & 0xFFFF
            if signed_word(previous_turn ^ roll_after) < 0:
                roll_after >>= 1
                motion_after = roll_after & 0xFFFF
            ...
            heading_carry = ((roll_after & 0xFFFF) >> 4) & 1
            pan_after = (pan + heading_delta + heading_carry) & 0xFFFF
            counter_roll = (-roll_after) & 0xFFFF
            follower_pan = signed_word(counter_roll) >> 1
            follower_roll = signed_word(counter_roll) >> 2
            put_u16(data_expected, state + 0x50, pan_after)
            put_u16(data_expected, state + 0x52, roll_after)
        ...
        if machine.reg_read(UC_X86_REG_SP) != 0xFF02:
            raise AssertionError(f"{module}:{entry:#x} {name}: stack pointer changed")
        if bool(machine.reg_read(UC_X86_REG_EFLAGS) & 1) != turn_applied:
            raise AssertionError(f"{module}:{entry:#x} {name}: carry result changed")
        ...
        vectors.append(
            {
                ...
                "module": module,
                "entry": entry,
                "precision": "damping" if shift == 20 else "approach",
                "turn_applied": turn_applied,
                "camera_x_before": camera_x & 0xFFFF,
                "camera_z_before": camera_z & 0xFFFF,
                "forward_x_before": forward_x & 0xFFFFFFFF,
...
def alien_slot1_state_update_vectors(
    ...
    """Exercise every branch in the slot-1 bounds and selection callback."""
    image = load_image(module)
    if hashlib.sha256(image[entry : entry + body_size]).hexdigest() != body_hash:
        raise AssertionError(f"{module}:{entry:#x}: recovered slot-1 state callback changed")
    ...
    camera_call = 0xCA11
    cases = (
        ("depth_high", 0, 0, 129, 0, False, ("bounds", "depth_high")),
        ("lateral_high", 65, 0, 0, 0, False, ("bounds", "lateral_high")),
        ("lateral_low", -65, 0, 0, 0, False, ("bounds", "lateral_low")),
        ("vertical_high", 0, 65, 0, 0, False, ("bounds", "vertical_high")),
        ("vertical_low", 0, -65, 0, 0, False, ("bounds", "vertical_low")),
        ("selection_requested", 0, 0, 0, 1, False, ("selection_busy", "requested")),
        ("selection_complete", 0, 0, 0, 2, False, ("selection_busy", "complete")),
        ("ready", 0, 0, 0, 0, False, ("ready",)),
        ("ready_wrapping_values", 64, -64, 128, 0, True, ("ready", "boundaries", "wrap")),
    )
    ...
    for case_index, (
        ...
        code_before = bytearray(image)
        code_before[return_address] = 0xCC
        install_stub(code_before, motion_target, motion_call)
        install_stub(code_before, camera_target, camera_call)
        code_expected = bytearray(code_before)
        ...
        for field, value in (
            (0x0000, 0x4444),
            (0x000E, 0x5555),
            (0x0038, translation_x),
            (0x003C, translation_y),
            (0x0040, translation_z),
            (0x0054, 0x6666),
        ):
            put_u16(data_before, state + field, value)
            put_u16(data_expected, state + field, value)
        position_before = (
            0xFFFF1234 if wrapping_values else 0x11223344,
            0x80005678 if wrapping_values else 0x55667788,
            0x7FFF9ABC if wrapping_values else 0x99AABBCC,
        )
        for field, value in zip((0x0042, 0x0046, 0x004A), position_before):
            put_u32(data_before, state + field, value)
            put_u32(data_expected, state + field, value)
        pulse_before: list[int] = []
        for pulse_index, (offset, _amount) in enumerate(pulse_updates):
            value = (
                0xFFFFFFF0 + pulse_index
                if wrapping_values
                else 0x10203040 + offset
            )
        ...
        expected_action = "selected"
        expected_call = no_call
        if outside_bounds:
            expected_action = "motion"
            expected_call = motion_call
        else:
            put_u16(data_expected, 0x2282, context if context_latch else 1)
            if selection & 3:
                expected_action = "camera"
                expected_call = camera_call
            else:
                put_u16(code_expected, selection_offset, 1)
                put_u16(data_expected, state + 0x0000, 0x25A8)
                put_u16(data_expected, state + 0x0054, 0)
                put_u16(data_expected, state + 0x0042, 0)
                put_u16(data_expected, state + 0x0046, 0)
                put_u16(data_expected, state + 0x004A, 32)
                for offset, amount in pulse_updates:
                    put_u32(data_expected, offset, get_u32(data_expected, offset) + amount)
                put_u16(data_expected, state + 0x000E, wave_callback)
                put_u16(data_expected, 0x001E, 5)
        ...
        vectors.append(
            {
                ...
                "selection_after": struct.unpack_from(
                    "<H", code_expected, selection_offset
                )[0],
                "control_latch_after": get_u16(data_expected, 0x2282),
                "callback_countdown_after": get_u16(data_expected, 0x001E),
                "parent_after": get_u16(data_expected, state + 0x0000),
                "callback_after": get_u16(data_expected, state + 0x000E),
                "radial_after": get_u16(data_expected, state + 0x0054),
                "position_before": list(position_before),
                "position_after": [
                    get_u32(data_expected, state + field)
                    for field in (0x0042, 0x0046, 0x004A)
                ],
...
def main() -> int:
    ...
    for (
        ...
        (
            "amer", 0x0BEA, 115,
            "88a0152008a8470b6d96b9870b07602907490baa31488c22e7de1ba833e2cb57",
            0x0B2F, 0x0B37, 0x0CAC, 0x0C5D,
            ((0x2594, 30), (0x25F2, 35)), False,
        ),
        (
            "croolis", 0x0C3E, 119,
            "7c43e7c97392048a7cd8a9b40ebee22d51ce74cceb5a6630d34d5c52075e8de5",
            0x0B70, 0x0B78, 0x0D04, 0x0CB5,
            ((0x2536, 25), (0x2594, 30), (0x25F2, 35)), True,
        ),
        (
            "scrut", 0x0C32, 113,
            "9a12f55ba875f386f42d1619af6ee241afc7e0957b4436506062d58d27572071",
            0x0B70, 0x0B78, 0x0CF2, 0x0CA3,
            ((0x2594, 30), (0x25F2, 35)), True,
        ),
    ):
        update_vector(
            VECTOR_ROOT / f"xdb_{module}_func_{entry:04x}_natural.json",
            alien_slot1_state_update_vectors(
                module,
                entry,
                body_size,
                body_hash,
    ...
    update_vector(
        VECTOR_ROOT / "xdb_amer_func_1a2b_natural.json",
        amer_slot2_reset_vectors(0x1A2B),
        args.check,
    )
    for entry, body_hash, next_entry, radial_target, next_stage in (
        (
            0x1688,
    ...
    update_vector(
        VECTOR_ROOT / "xdb_amer_func_171d_natural.json",
    ...
    update_vector(
        VECTOR_ROOT / "xdb_croolis_func_171d_natural.json",
        croolis_slot2_restart_vectors(0x171D),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_croolis_func_1727_head_natural.json",
        croolis_slot2_update_head_vectors(0x1727),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_croolis_func_1815_natural.json",
        croolis_slot2_selection_init_vectors(0x1815),
        args.check,
    )
    update_vector(
    ...
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1711_natural.json",
        scrut_slot2_restart_vectors(0x1711),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_171b_head_natural.json",
        scrut_slot2_update_head_vectors(0x171B),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1781_natural.json",
        scrut_slot2_common_dispatch_vectors(0x1781),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1787_natural.json",
        scrut_slot2_motion_vectors(0x1787),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_17e1_natural.json",
        scrut_slot2_begin_fade_vectors(0x17E1),
        args.check,
    )
    update_vector(
    ...
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1952_natural.json",
        scrut_slot2_finish_setup_vectors(0x1952),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1957_natural.json",
        scrut_slot2_finish_update_vectors(0x1957),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_19cf_natural.json",
        scrut_slot2_selection_reset_vectors(0x19CF),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1a03_natural.json",
        scrut_slot2_active_reset_setup_vectors(0x1A03),
        args.check,
    )
    update_vector(
        VECTOR_ROOT / "xdb_scrut_func_1a11_natural.json",
    ...
    for module, entry in (
        ...
        update_vector(
            VECTOR_ROOT / f"xdb_{module}_func_{entry:04x}_natural.json",
            alien_resume_texture_motion_vectors(module, entry),
            args.check,
        )
    for module, entry in (
        ("amer", 0x1CBF),
        ...
        update_vector(
            VECTOR_ROOT / f"xdb_{module}_func_{entry:04x}_natural.json",
            alien_resume_timeout_vectors(module, entry),
            args.check,
        )
    for module, entry in (
        ("amer", 0x1CFA),
