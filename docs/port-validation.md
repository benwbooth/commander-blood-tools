...
| hand poses (selector sequences) | **DECODED-EXACT mapping** | the 0x181 dispatch decoded: sequence = table[ds:[0x2306]=0x2974] + table[(sel&0x1F)*2] — 17 distinct sequences, one per selector 0..0x10 (>=0x11 alias 0's no-op); PosePlayer now loads by SELECTOR through the game's own table from the ds dump (replacing the guessed repack indexing). Selector = event pulse (SELECTORWATCH). Remaining: per-sequence visual confirmation vs live captures (attract/story frontier) |
