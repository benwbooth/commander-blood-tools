...
| hand poses (selector sequences) | **model confirmed; sequences partially verified** | SELECTORWATCH (banked probe): [0xA32] reads 0 in ALL hub interaction contexts -> the selector is an EVENT PULSE (write-then-consume), matching the engine's one-shot hand_pose_event design; the persistent pose = whatever the last event selected, articulated by tweens. Rest pose = the dump state (oracle-exact vs HANDGRID). Individual sequence visuals per selector remain unverified (need deep-game states) |
