...
| main.rs console_box row 2 | BOB_MORLOCK / CANCEL contact menu | **OPEN — TRANSCRIBED LABELS** | The two labels are written into `main.rs` unconditionally. Both are DIC words in the game's own dictionary (`Bob_Morlock` in SCRIPT2/3/4/5.DIC), so the list is content that lives in the data. The port already has the parser (`bas_vm::parse_menu_block` — the `0xA3` menu head's word list plus its `0xA6` responses), and it is called from nowhere. THE TASK: find the menu record the console row-2 contact opens and drive the labels from it. Guarded by `tools/check_content_literals.py` (KNOWN_OPEN) so no NEW label list can appear |
| main.rs OPTION menu | TEXT / MUSIC_OFF / SAVE / LOAD / QUIT / CANCEL | **OPEN — TRANSCRIBED LABELS** | Same defect, six labels. Each is a DIC word (`music_off`, `save`, `load`, `text` present across SCRIPT1..5.DIC). Same fix and same guard entry |
