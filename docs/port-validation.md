...
| audio.rs mixing | simultaneous sounds | **WIRED (audible check pending)** | The game MIXES by averaging: `0xBB6D..0xBB74` is `lodsb / add al,es:[di] / rcr al,1 / stosb`, where the add's carry becomes bit 7 during the rotate, giving `floor((src+dst)/2)` for two u8 samples. Ported as `snd::snd_mix_average` + `mix_unsigned_pcm_average`, called by nothing. The port instead runs THREE independent `MusicPlayer` streams (music, voice, chatter) and lets the audio backend sum them — so sources play at full amplitude and can clip, where the game halves each and cannot. Audible, not cosmetic. Closing it needs a mixing output PATH. The rule is now decoded and tested at both levels — `snd_mix_average` (one source, all 65536 input pairs against `add`/`rcr`) and `mix_unsigned_pcm_sources` (N sources, order-dependent by construction, silence = `0x80`) — so what remains is rewiring `audio.rs` from three independent cpal streams to one mixed stream. Not done here: this environment has no audio device, and an unverifiable change to the output path is exactly the kind this campaign should not make on faith HALF-RATE VARIANT NOW DECODED (this session): `0xBB53 test byte gs:[0xba2],1` chooses between TWO mix loops. `0xBB6D` is the normal one; `0xBB5B` reads without advancing and does `inc si` only when the loop counter is EVEN (`test cl,1 / jne` @`0xBB5D`), consuming the source once per two output samples -- the voice plays at half sample rate through the IDENTICAL average. Ported as `snd::mix_unsigned_pcm_half_rate`, tested on both counter phases (an even count gives `A,B,B,C,C`, an odd count `A,A,B,B,C`). WHAT STILL BLOCKS THE REWIRING, now stated exactly: `0xBB21 les di,[bp] / add di,6` shows the routine mixes into a buffer owned by a voice STRUCT, so whether a lone sound is halved toward silence or written at full amplitude depends on who initialises that buffer per frame -- undecoded. `mix_unsigned_pcm_sources` assumes a silence pre-fill, which is why the live path is NOT yet switched to it: wiring an unverified initialisation would change every sound's amplitude on faith. The decode to do next is the buffer's per-frame owner (the `gs:[0xcf3]` far call @`0xBB28` is the lead) **MODEL CORRECTION (this session), and it undercuts this row's own premise.** The prologue shows the mixer is a STREAMER, not an N-source summer: `0xBAF7 mov ah,0x3F / int 0x21` reads the next chunk from a FILE, `0xBAFD sub cx,6` drops a 6-byte header, and it targets whichever of exactly TWO voice structs (`0xB89`, `0xB91`) is in state 3 (`cmp byte [bp+6],3`). The write lands at an offset from the device play position (`lcall gs:[0xcf3]` @`0xBB28`) and is clamped to the buffer, with the leftover wrapping at `0xBB76 or bx,bx` -- a RING BUFFER written in up to two spans, ported+tested as `snd::stream_mix_span`. So `mix_unsigned_pcm_sources(N sources, silence pre-fill)` describes a mixing model the game does not have: nothing here sums three simultaneous streams, and the silence pre-fill it assumes is an invention. That function stays as the decoded ELEMENT-WISE rule (`0xBB6D`) it was tested against, but it is NOT the wiring target, and this row no longer claims the port should switch to it. `0xBB0B add cx,cx` also independently confirms the half-rate decode: the same `gs:[0xBA2]` flag that picks the `0xBB5B` loop doubles the output count. **ANSWERED (`0xBBE4..0xBC2F`): the LOADED SOUND DATA.** `les di,[0xbb7]` points voice A at the file itself, length `0x4000`, and voice B starts `0x4008` later -- the two voices are the data's two halves with the 8-byte header between them. NOTHING pre-fills silence, so a lone sound is NOT halved toward `0x80`; `0xBB6D` averages an incoming chunk with sound already there. That closes the amplitude question this row was blocked on. AND the half-rate flag is DATA: `cmp byte es:[di+4],0xd3` @`0xBBFE` sets `gs:[0xBA2]` @`0xBC05` from the sound file's own header byte (`0xD3` = the SB time constant for 22222 Hz). Ported as `snd_header_is_half_rate` + `snd_voice_buffer_spans`, tested. The port must READ that byte rather than choose a rate -- hardcoding the decision would be exactly the content-bearing literal CLAUDE.md forbids. **THE STRUCTURAL CORE IS NOW BUILT AND TESTED**: `snd::SndStream` = two voices sliced from the loaded data, a half-rate flag read from its header, `active_voice` (state 3, first-preferred per the `xchg` @`0xBB19`), and `mix_chunk` composing `stream_mix_span` + the two mix loops including the `0xBB76` wrap. Device-independent BY DESIGN, which is what makes it verifiable here at all -- the no-audio-device objection applied to the OUTPUT PATH, never to the rule. Its tests caught a real omission: `add di,6` @`0xBB24` mixes PAST a 6-byte header in each voice buffer (`SND_VOICE_HEADER_LEN`), which is also why voice B sits at `0x4008` rather than `0x4000`. The core is now checked against REAL GAME DATA: `snd_stream_mixes_a_real_clip_from_the_shipped_bank` takes a clip out of `sn/tb.snd` and asserts every mixed sample equals `snd_mix_average(clip, original)` -- the decoded rule on the game's own bytes, not on a pattern picked to pass. It also ties `SND_HEADER_HALF_RATE_TIME_CONSTANT` to the independently decoded `snd_sample_rate`: `0xD3` -> 22222 Hz. REMAINING, and now precisely bounded: `audio.rs` drives cpal with three `MusicPlayer` streams, and migrating it needs the DOUBLE-BUFFER SWITCH rule -- when the driver flips voice A/B and what advances the play cursor `gs:[0xcf3]` reports. **THE POSITION CALL IS NOW DECODED.** The driver vector slots are STATIC far pointers at `DS:0x0CD3`, nine of them four bytes apart with offsets `0x100 + 3k` -- `3` because a `.drv` opens with `E9 rel16` jumps and loads COM-style at `0x100`, so slot k is vector k. Nothing writes them, which is why no write site was ever found. `lcall gs:[0xcf3]` is VECTOR 8, and in `dnsdb.drv` (`DRV:0x01CA`) it reads the 8237 DMA controller's CURRENT COUNT (`dl = cs:[0x49d]` the channel, port = `channel*2 + 1`, two `in`s and an `xchg`). So the play position is the DMA's REMAINING count and it counts DOWN -- which is what `sub ax,[bp+4] / neg ax` @`0xBB33` converts into a played-offset, and why `stream_mix_span` takes an absolute difference. `lcall [0xcdf]` is vector 3. Pinned by `driver_vector_slots_are_static_far_pointers_to_the_drv_table`, which reads BOTH binaries. The host cursor now exists as `SndStream::dma_remaining_equivalent`, labelled INFRA rather than ASM because it is PORT PLUMBING standing in for a chip the port does not have: `remaining = length - (played mod length)`, derived to feed the arithmetic `stream_mix_span` already implements. Tested to walk the buffer forward and wrap exactly as a DMA countdown would. Keeping the substitution inside ONE function is what stops the invention spreading into the decoded mixing rules. **DONE.** `audio.rs` now opens ONE shared cpal stream; every `MusicPlayer` is a handle on a process-wide `AudioMixer` whose callback folds active sources with `snd::mix_unsigned_pcm_layered` -- the game's rule, first source unattenuated (the loader's overwrite @`0x4049`) and later ones averaged in (`0xBB6D`). The three independent streams the OS used to sum at full amplitude are gone, and with them the clipping the original cannot produce. The public API did not change, so every caller in `main.rs` gained this without edits. `AudioMixer::render` is device-free by construction and tested: silence when idle, a lone source unattenuated, two sources averaged sample-by-sample, play-once reaped and loops not. WHAT A DEVICE IS STILL NEEDED FOR: confirming it SOUNDS right. Not for knowing the mixing is right |
