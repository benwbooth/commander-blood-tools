...
| engine.rs hand cursor | pointing-hand | **CAPTURE(dense)** | 42-pose grid atlas captured from the live hub (HANDGRID; modal-background diff, per-sprite live RGB). Nearest-of-42 closely tracks the real 3D render; the full manu3 mesh render remains the ideal endgame. Presentations draw NO cursor; interactive screens show the hand |
