...
| engine.rs hand cursor | pointing-hand | **APPROX (stopgap)** | THE fix is porting manu3's real 3D render — the capture atlas (42 poses) is an interim stand-in, NOT the port target (user: "this is supposed to be an accurate port"). Verified footholds for the decompile: manu3.xdb starts with CODE (its own renderer); transform at file+0x477 = Q15 3x3 matrix at ds:0x2250 over dword vertices; trig tables at 0x17A3/0x1C94/0x2094/0xA482; TEXTURE pixel data (teal ramp d5/f5 runs) at ~0x6480+; rasterizer write-sites 0x2AF..0x13xx; shared 3D core also in amer/croolis/scrut.xdb. Task: trace from the entry handlers to the per-frame hand draw -> mesh pointer -> faces/UVs -> port transform+affine rasterizer |
