...
| engine.rs bridge starfield | stars behind the panorama windows | **OPEN — MEASURED DIVERGENCE** | `nav_screen_opened.ppm` (captures/bridge/) shows the bridge at the nav station with a dense white starfield filling the top ~135 rows and the console band below. The port renders the panorama with BLACK windows: `render_bridge_background` composites starfield-then-panorama (colour 0 transparent), but the star layer produces nothing on the headless path — `render_ship_3d_starfield` returns `None`, or only the `gpu_hand_enabled` branch fills it. mean_abs 102 vs the capture. The point cloud itself IS decoded (`randomize_ship_3d_point_cloud`, `SHIP_3D_POINT_CLOUD_LEN` 1000 @`0x9B6A`, base `DS:0x2FC1` @`0x9B71`) and oracle-settled; this is a WIRING gap, not a decode gap. Measured by `nav_screen_render_distance` (ignored — a tolerance that accommodated this would encode the bug) |
