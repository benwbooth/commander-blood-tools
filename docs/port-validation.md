...
| engine.rs hand cursor | pointing-hand 3D model | **ASM+DATA (live, exact)** | THE REAL 3D HAND, live as the cursor: mesh(142v/216f)+UVs(vert +0/+2)+256-wide texture extracted from the running game; EXACT Q14 matrix build (trig ds:0x26), EXACT perspective projection (0x549), z-buffer visibility, top-left edge ownership; 9 decoded POSES driven by the exact tween player (0x1DF/0x19B
...
| engine.rs cyberspace | BIOXX minigame | **APPROX (precise gap)** | the port plays hyper_*.hnm (the hyperspace TRAVEL videos) as a steer-through-tunnel; but cyberspace proper = the cyber.ext / cyber2/cyber3 worlds (1cyber*.lbm rooms + BIOXX entities), reached via the decoded world-visit list system (goal: touch BIOXX -> BIONIUM). The ONE genuine open behavior: route cyberspace through the world-visit system on the cyber worlds (not the travel HNMs) + confirm against a playthrough capture that reaches it. Needs the story-playthrough driver to observe the real cyberspace screen |
