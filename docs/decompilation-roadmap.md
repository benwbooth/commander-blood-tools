...
| 3 | Ship/nav UI + mouse/keyboard | **Done** (capability, tested) | enters the real bridge/nav UI (TB.BIG panorama) and responds to mouse (decompiled steering/seek/menu, `src/bridge.rs`) + keyboard; console at pixel-parity 0.14; choose-a-location destination list — **test `nav_destination_list_choose_a_location` passes**, and the offered destinations (SCRIPT3/4/5 hosts) **match the binary's D2 handoff targets** (verified). Depth residual (secondary 3D-anchor rendering only): exact pyramid-anchor *positions*, gated on a story-granted world-load state; the full activation mechanism (0x8CCE→0x40D0→0x9B98) + the `what`-topic dispatch are decoded, a clean savestate is ready, and the sole remaining unknown is SCRIPT2's conversation-flow path to the `what` topic (a fresh-session VM-flow RE task) — the port's navigation itself is faithful |
