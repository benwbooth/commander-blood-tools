...
pub(super) fn decompress_lz_171(data: &[u8], offset: usize) -> Result<Vec<u8>, Box<dyn Error>> {
...
pub(super) struct BitReaderHigh<'a> {
    pub(super) data: &'a [u8],
    pub(super) pos: usize,
    pub(super) bits_left: u32,
    pub(super) queue: u16,
}
...
impl<'a> BitReaderHigh<'a> {
    ...
    pub(super) fn get_bit(&mut self) -> u8 {
        if self.bits_left == 0 {
    ...
    pub(super) fn get_byte(&mut self) -> u8 {
...
pub(super) fn decompress_rle_173(data: &[u8], offset: usize) -> Result<Vec<u8>, Box<dyn Error>> {
