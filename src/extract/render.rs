...
// THE FONT LIVES IN `crate::font`. This module used to carry its own copy of the
// glyph struct, the lookup, the advance AND all three tables — and that copy still
// had three defects the shared one has fixed:
//   * `GAME_FONT_CHAR_MAP` truncated to 128 entries (the real table is 176; see
//     audit-fixes #56), so every accented character was unmapped;
//   * indexed by `ch as usize`, a Unicode scalar, where the table is indexed by
//     CP437 BYTE — so `é` (U+00E9 = 233) fell past the end even at 176;
//   * unmapped characters fell back to `'?'`, where `render_string` (`0x31CE`
//     `xlatb / or al,al / js`) skips them with no glyph AND no advance.
// Delegating means the extraction path renders what the game renders.
pub(super) type GameFontGlyph = commander_blood_tools::font::GameFontGlyph;
...
pub(super) fn game_font_advance(ch: char) -> usize {
    commander_blood_tools::font::game_font_advance(ch)
}
...
mod tests {
    ...
    #[test]
    fn subtitle_glyphs_follow_render_string_not_a_question_mark_fallback() {
        // A space draws NO glyph...
        assert!(subtitle_draw_glyph(' ').is_none());
        // ...a known character draws itself...
        assert_eq!(subtitle_draw_glyph('M'), game_font_glyph('M'));
        // ...and an UNMAPPED character draws NOTHING. `render_string` @0x31CE does
        // `xlatb / or al,al / js` — no glyph and no advance. This test used to
        // assert a '?' fallback, which was this module's private copy of the font
        // inventing a character the game cannot draw; it now shares crate::font.
        assert!(subtitle_draw_glyph('\u{2603}').is_none());
        assert!(game_font_glyph('?').is_some(), "'?' itself is a real glyph");
        // And the shared table is the 176-entry one, so accented characters map.
        assert!(
            game_font_glyph('é').is_some(),
            "the private copy truncated the map at 128 and lost these"
        );
    }
